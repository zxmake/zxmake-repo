package("expat", function()
    set_homepage("https://libexpat.github.io")
    set_description("XML 1.0 parser")
    set_license("MIT")

    set_urls(
        "https://github.com/libexpat/libexpat/releases/download/R_$(version).tar.bz2",
        {
    ...
    add_configs("char_type", {
        description = "Character type to use",
        default = "char",
        type = "string",
    ...
    on_install(function(package)
        ...
        table.insert(configs, "-DEXPAT_SHARED_LIBS=" ..
                         (package:config("shared") and "ON" or "OFF"))
        table.insert(configs,
                     "-DEXPAT_CHAR_TYPE=" .. package:config("char_type"))
        if package:is_plat("windows") then
            table.insert(configs, "-DEXPAT_MSVC_STATIC_CRT=" ..
