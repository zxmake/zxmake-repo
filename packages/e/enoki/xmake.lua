package("enoki", function()
    ...
    add_configs("cuda", {
        description = "Build Enoki CUDA library",
        default = false,
        type = "boolean"
    })
    add_configs("cudacc", {
        ...
        type = "string",
        values = {
            "35", "50", "52", "60", "61", "70", "75", "80", "86", "89", "90"
        }
    })
    add_configs("autodiff", {
        description = "Build Enoki automatic differentation library",
        default = false,
        type = "boolean"
    })
    add_configs("python", {
        description = "Build pybind11 interface to CUDA & automatic differentiation libraries",
        default = false,
        type = "boolean"
    })
    ...
    on_install("windows", "macosx", "linux", "mingw", function(package)
        ...
        if package:config("cuda") or package:config("autodiff") then
            ...
            table.insert(configs, "-DENOKI_CUDA=" ..
                             (package:config("cuda") and "ON" or "OFF"))
            if package:config("cuda") then
                table.insert(configs, "-DENOKI_CUDA_COMPUTE_CAPABILITY=" ..
                                 package:config("cudacc"))
            end
            table.insert(configs, "-DENOKI_AUTODIFF=" ..
                             (package:config("autodiff") and "ON" or "OFF"))
            table.insert(configs, "-DENOKI_PYTHON=" ..
