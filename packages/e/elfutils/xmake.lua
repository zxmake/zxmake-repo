package("elfutils", function()
    ...
    add_patches("0.183", path.join(os.scriptdir(), "patches", "0.183",
                                   "configure.patch"),
                "7a16719d9e3d8300b5322b791ba5dd02986f2663e419c6798077dd023ca6173a")
    add_patches("0.189", path.join(os.scriptdir(), "patches", "0.189",
                                   "configure.patch"),
                "b4016a97e6aaad92b15fad9a594961b1fc77a6d054ebadedef9bb3a55e99a8f8")

    add_configs("libelf", {
        description = "Enable libelf",
        default = true,
        type = "boolean"
    })
    add_configs("libdw", {
        description = "Enable libdw",
        default = true,
        type = "boolean"
    })
    add_configs("libasm", {
        description = "Enable libasm",
        default = false,
        type = "boolean"
    })
    ...
    if on_check then
        ...
        on_check("android", function(package)
            ...
            assert(ndkver and tonumber(ndkver) < 26,
                   "package(elfutils): need ndk version < 26 for android")
            assert(ndk_sdkver and tonumber(ndk_sdkver) <= 23,
                   "package(elfutils): need ndk api level <= 23 for android")
        end)
    end
    ...
    on_install("linux", "android", function(package)
        local configs = {
            ...
            "--program-prefix=elfutils-", "--disable-symbol-versioning",
        ...
        for _, makefile in ipairs(os.files(path.join("*/Makefile.in"))) do
            ...
            if package:has_tool("cc", "clang") then
                io.replace(makefile, "-Wno-packed-not-aligned", "",
                           {plain = true})
            end
        end
        local subdirs = {}
        ...
        if package:config("libdw") then
            table.join2(subdirs, "libcpu", "backends", "libebl", "libdwelf",
                        "libdwfl", "libdw")
        end
	  ...
	  libasm debuginfod src po doc tests]],
                   "SUBDIRS = lib " .. table.concat(subdirs, " "),
                   {plain = true})

        if package:is_plat("android") then
            io.replace("libelf/Makefile.in",
                       "-Wl,--whole-archive $(libelf_so_LIBS) -Wl,--no-whole-archive",
                       "$(libelf_so_LIBS)", {plain = true})
            io.replace("libdw/Makefile.in",
                       "-Wl,--whole-archive $(libdw_so_LIBS) -Wl,--no-whole-archive",
                       "$(libdw_so_LIBS)", {plain = true})
            io.replace("libasm/Makefile.in",
                       "-Wl,--whole-archive $(libasm_so_LIBS) -Wl,--no-whole-archive",
                       "$(libasm_so_LIBS)", {plain = true})
