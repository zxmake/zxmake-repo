package("qtifw", function()
    ...
    on_fetch(function(package, opt)
        if opt.system then
            return import("lib.detect.find_tool")("binarycreator",
                                                  {check = "--help"})
        end
    end)
    ...
    on_install("@windows", "@msys", "@macosx", function(package)
        ...
        local qtifw_version = "qt.tools.ifw." .. version:major() ..
                                  version:minor()
        if is_host("windows") and (not is_subhost("msys")) then
            os.vrunv("aqt", {
                "install-tool", "-O", installdir, host, target, "tools_ifw",
                qtifw_version, "--external", "7z"
            })
        else
            os.vrunv("aqt", {
                "install-tool", "-O", installdir, host, target, "tools_ifw",
        ...
        os.mv(path.join(installdir, "Tools", "*",
