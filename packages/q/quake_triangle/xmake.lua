package("quake_triangle", function()
    ...
    add_configs("single", {
        description = "Use single-precision float.",
        default = false,
        type = "boolean"
    })

    on_install(function(package)
        io.replace("triangle.h", "REAL",
                   package:config("single") and "float" or "double",
                   {plain = true})
        io.replace("triangle.h", "#ifdef ANSI_DECLARATORS", "#if 1",
                   {plain = true})
        io.replace("triangle.h", "VOID", "void", {plain = true})
        io.replace("triangle.c", "#define VOID int", "#define VOID void",
                   {plain = true})
        local xmake_lua = [[
