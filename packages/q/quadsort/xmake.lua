package("quadsort", function()
    ...
    if on_check then
        on_check(function(package)
            if package:version():ge("1.2.1+3") then
                assert(not package:has_tool("cxx", "cl"),
                       "package(quadsort 1.2.1+3) unsupported msvc")
            end
        end)
    end
    ...
    on_test(function(package)
        assert(package:has_cfuncs("quadsort",
                                  {includes = {"string.h", "quadsort.h"}}))
    end)
end)
