...
on_install(function(package)
    ...
    os.vrunv("aqt",
             {"install-qt", "-O", installdir, host, target, versionstr, arch})
    ...
    if package:is_cross() then
        ...
        os.vrunv("aqt", {
            "install-qt", "-O", path.join(installdir, "bin_host"), runhost,
            "desktop", versionstr, hostarch
        })

        -- add symbolic links for useful tools
        local tool_folders = {}
        if version:ge("6.0") then
            tool_folders.bin = {qmake = true, qmake6 = true}

            tool_folders.libexec = {moc = true, rcc = true, uic = true}
        else
            tool_folders.bin = {
                qmake = true,
                moc = true,
        ...
        for folder, tools in pairs(tool_folders) do
            for _, file in ipairs(os.files(
                                      path.join(installdir, "bin_host",
                                                versionstr, "*", folder, "*"))) do
