package("qdcae", function()

    set_homepage("https://github.com/qd-cae/qd-cae-python")
    set_description(
        "qd python (and C++) library for CAE (currently mostly LS-Dyna) ")

    set_urls("https://github.com/qd-cae/qd-cae-python/archive/$(version).zip",
             "https://github.com/qd-cae/qd-cae-python.git")
    add_versions("0.8.9",
                 "d5e838a7685d3407919c3e6ad33a17a8bfe376df26dbcc74a5368d8e8fd64c4c")
    add_patches("0.8.9", path.join(os.scriptdir(), "patches", "fix.patch"),
                "b865ac7c0db7bd792aa536a9ca8b7d504b04cce7e7aea2073dbc00e276f5b8fc")
    ...
    on_install("linux", "windows", "mingw", "bsd", function(package)
        ...
        os.cp("qd/cae/dyna_cpp/**.h",
              path.join(package:installdir(), "include"), {rootdir = "qd/cae"})
        os.cp("qd/cae/dyna_cpp/**.hpp",
              path.join(package:installdir(), "include"), {rootdir = "qd/cae"})
    end)
