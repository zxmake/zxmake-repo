package("workspace_hanya", function()

    set_kind("library", {headeronly = true})
    set_homepage("https://github.com/CodingHanYa/workspace")
    set_description(
        "Workspace is a lightweight asynchronous execution framework based on C++11")
    ...
    on_install("windows", "linux", "macosx", "mingw", function(package)
        if is_plat("linux") then
            io.replace("CMakeLists.txt",
                       "install(DIRECTORY ${LIB_HEADER} DESTINATION ${INSTALL_INCLUDEDIR}/${LIB_NAME})",
