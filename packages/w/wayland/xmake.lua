package("wayland", function()
    ...
    add_resources("1.18.0", "protocols",
                  "https://wayland.freedesktop.org/releases/wayland-protocols-1.20.tar.xz",
                  "9782b7a1a863d82d7c92478497d13c758f52e7da4f197aa16443f73de77e4de7")
    add_resources("1.19.0", "protocols",
    ...
    add_resources("1.23.0", "protocols",
    ...
    add_deps("meson", "ninja >=1.8.2", "libxml2", "libffi", "expat", "bison",
             "pkg-config")
    ...
    on_install("linux", "bsd", function(package)
        ...
        local PKG_CONFIG_PATH = path.joinenv({
            package:installdir("lib", "pkgconfig"),
            package:installdir("share", "pkgconfig")
        })
        ...
        envs.LD_LIBRARY_PATH = path.joinenv(
                                   table.join(LD_LIBRARY_PATH,
                                              envs.LD_LIBRARY_PATH))
        envs.PKG_CONFIG_PATH = path.joinenv(
                                   table.join(PKG_CONFIG_PATH,
                                              envs.PKG_CONFIG_PATH))
        envs.ACLOCAL_PATH = path.joinenv(
                                table.join(ACLOCAL_PATH, envs.ACLOCAL_PATH))
        envs.ACLOCAL = ACLOCAL
        meson.install(package, configs, {envs = envs})

        -- install wayland-protocols
        local protocol = assert(package:resourcedir("protocols"),
        ...
        if buildfile then
            ...
            local configfile = assert(find_file("configure.ac",
                                                path.join(protocol, "**")))
