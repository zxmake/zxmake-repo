package("hpsocket", function()
    ...
    add_versions("v5.7.3",
                 "e653f3c15ded3a4b622ab9a4a52a477c7aa40f5b86398c6b75f5a732a55496a0")
    add_versions("v5.8.4",
                 "6fd207b84e41174c06d27c0df7244584eb07fbac0a7e49d7429103071184a451")
    add_versions("v5.9.1",
                 "d40a3d0b4f0d2773ae61d32ed95df655aa6ccf5ae22c40ef38bfc88882b2478b")

    local configs = {
        {name = "udp", package = "kcp"},
        {name = "http", package = "http_parser"},
        {
            name = "zlib",
            package = is_plat("android", "windows") and "" or "zlib"
    ...
    for _, cfg in ipairs(configs) do
        ...
        add_configs(cfg_name, {
            description = "Build hpsocket without " .. cfg.name,
            default = false,
            type = "boolean"
        })
    end
    add_configs("no_4c", {
        description = "Build hpsocket without C interface",
        default = true,
        type = "boolean"
    })
    add_configs("unicode", {
        description = "Build hpsocket with unicode character set",
        default = false,
        type = "boolean"
    })
    ...
    on_load(function(package)
            ...
            if not package:config(cfg_name) then
                if cfg.package ~= "" then
                    package:add("deps", cfg.package,
                                package:is_plat("windows") and {} or
                                    {configs = {cxflags = "-fpic"}})
                end
            else
                package:add("defines",
                            "_" .. string.upper(cfg.name) .. "_DISABLED")
            end
        end
        ...
        package:add("links",
                    package:config("no_4c") and "hpsocket" or "hpsocket4c")
        if not package:config("shared") then
            if not package:config("no_ssl") then
    ...
    on_install("windows|x64", "windows|x86", "linux", "android",
               function(package)
    ...
    on_test(function(package)
        assert(package:check_cxxsnippets({
            ...
            includes = package:config("no_4c") and "HPSocket.h" or
                "HPSocket4C.h"
        }))
    end)
end)
