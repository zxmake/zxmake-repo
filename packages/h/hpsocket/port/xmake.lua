local dep_packages = {}
local options = {
    {name = "udp", package = "kcp"}, {name = "http", package = "http_parser"},
    {name = "zlib", package = is_plat("android", "windows") and "" or "zlib"},
    {name = "brotli", package = "brotli"}, {name = "ssl", package = ""},
    {name = "iconv", package = ""}
}
local winCommonSrcPath = (get_config("hpversion") == "v5.7.3") and
                             "Windows/Common/Src/" or "Windows/Src/Common/"
local winBuiltinDependentLibPath = (get_config("hpversion") == "v5.7.3") and
                                       "Windows/Common/Lib/" or
                                       "Windows/Dependent/"
...
for _, opt in ipairs(options) do
    ...
    if not has_config(opt_name) and opt.package ~= "" then
        add_requires(opt.package, is_plat("windows") and {} or
...
exclude_file = no_4c and "HPSocket4C.*|HPSocket4C-SSL.*" or
                   "HPSocket.*|HPSocket-SSL.*"
...
if is_plat("windows") then
    ...
    add_defines(is_kind("shared") and "HPSOCKET_EXPORTS" or
                    "HPSOCKET_STATIC_LIB")
    ...
    add_includedirs(".")
    add_includedirs(path.join(winBuiltinDependentLibPath, "openssl", vs_ver,
                              "$(arch)", "include"))
    ssllinkdir = path.join(winBuiltinDependentLibPath, "openssl", vs_ver,
                           "$(arch)", "lib")
    add_linkdirs(ssllinkdir)
    add_includedirs(path.join(winBuiltinDependentLibPath, "zlib", vs_ver,
                              "$(arch)", "include"))
    zliblinkdir = path.join(winBuiltinDependentLibPath, "zlib", vs_ver,
                            "$(arch)", "lib")
