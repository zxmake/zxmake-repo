    ...
    on_fetch(function(package, opt)
        if opt.system then
            ...
            if package:is_plat("windows") then
                local keys = winos.registry_keys(
                                 "HKEY_LOCAL_MACHINE\\SOFTWARE\\Side Effects Software\\Houdini *.*.*")
                for _, key in ipairs(keys) do
                    table.insert(paths,
                                 winos.registry_query(key .. ";InstallPath"))
                end
            elseif package:is_plat("macosx") then
                for _, path in ipairs(os.dirs(
                                          "/Applications/Houdini/Houdini*.*.*")) do
            ...
            if result.sdkdir then
                ...
                local prog =
                    find_program("houdini", {paths = os.getenv("PATH")})
                if prog then
            ...
            for _, lib in ipairs(libs) do
                ...
                local linkinfo = find_library(libname, {result.sdkdir}, {
                    suffixes = "custom/houdini/dsolib"
                })
                if linkinfo then
                    table.insert(result.linkdirs, linkinfo.linkdir)
                    table.insert(result.links, libname)
                    if package:is_plat("windows") then
                        table.insert(result.libfiles, path.join(
                                         linkinfo.linkdir, libname .. ".lib"))
                        table.insert(result.libfiles, path.join(result.sdkdir,
                                                                "bin",
                                                                libname ..
                                                                    ".dll"))
                    end
            ...
            -- find headers
            local path = find_path(path.join("HAPI", "HAPI.h"), {result.sdkdir},
                                   {suffixes = path.join("toolkit", "include")})
            if path then
