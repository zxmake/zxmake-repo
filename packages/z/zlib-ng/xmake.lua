package("zlib-ng", function()
    ...
    add_urls(
        "https://github.com/zlib-ng/zlib-ng/archive/refs/tags/$(version).tar.gz",
    ...
    if on_check then
        on_check("windows", "mingw", function(package)
            ...
            if package:version():ge("2.1.5") and package:is_arch("arm.*") then
                ...
                if msvc then
                    ...
                    assert(vs_sdkver and
                               semver.match(vs_sdkver):gt("10.0.19041"),
                           "package(zlib-ng/arm >= 2.1.5): need vs_sdkver > 10.0.19041.0")
                end
            end

            if package:version():ge("2.1.6") and package:is_arch("i386") and
                package:is_cross() then
                assert(false,
                       "package(zlib-ng/i386 >= 2.1.6): Unsupported cross compilation")
            end
        end)
        ...
        on_check("android", function(package)
            if package:version():ge("2.1.5") and package:is_arch("arm.*") then
                assert(false,
                       "package(zlib-ng/android >= 2.1.5): Unsupported android and arm")
            end
        end)
    end

    on_install("windows", "macosx", "linux", "android", "mingw",
               function(package)
        local configs = {
        ...
        table.insert(configs,
                     "-DINC_INSTALL_DIR=" .. package:installdir("include"))
        table.insert(configs, "-DLIB_INSTALL_DIR=" .. package:installdir("lib"))
        table.insert(configs, "-DZLIB_COMPAT=" ..
                         (package:config("zlib_compat") and "ON" or "OFF"))
