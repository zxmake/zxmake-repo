package("zltoolkit", function()
    set_homepage("https://github.com/ZLMediaKit/ZLToolKit")
    set_description(
        "A lightweight network framework based on C++11 that utilizes thread pool technology to achieve high concurrent network I/O.")
    set_license("MIT")
    ...
    on_install("linux", "windows", "macosx", function(package)
        ...
        table.insert(configs, "-DENABLE_MYSQL=" ..
                         (package:config("mysql") and "ON" or "OFF"))
        table.insert(configs, "-DENABLE_OPENSSL=" ..
                         (package:config("openssl") and "ON" or "OFF"))
        io.replace("CMakeLists.txt", "add_subdirectory(tests)", "",
                   {plain = true})
        io.replace("CMakeLists.txt", "$ENV{HOME}/${PROJECT_NAME}/lib", "lib",
                   {plain = true})
        io.replace("CMakeLists.txt", "$ENV{HOME}/${PROJECT_NAME}/include",
                   "include", {plain = true})
