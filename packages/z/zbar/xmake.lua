package("zbar", function()
    ...
    add_configs("symbologies", {
        description = "Select symbologies to compile",
        default = {
            "ean", "databar", "code128", "code93", "code39", "codabar", "i25",
            "qrcode", "sqcode"
        },
        type = "table"
    })
    ...
    on_install("!iphoneos and !windows", function(package)
        ...
        for _, key in ipairs({"LIB_VERSION"}) do
            local value = configure_ac:match(
                              "AC_SUBST%(%[" .. key .. "%]%s*,%s*%[(.-)%]%)")
            if value then
                configs[key] = value
            end
