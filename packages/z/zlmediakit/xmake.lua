package("zlmediakit", function()
    ...
    add_configs("c_api", {
        description = "Enable C API SDK.",
        default = false,
        type = "boolean"
    })
    add_configs("c_static_api", {
        description = "Enable mk_api static lib.",
        default = false,
        type = "boolean"
    })
    ...
    add_configs("server_lib", {
        description = "Enable server as android static library.",
        default = false,
        type = "boolean"
    })
    ...
    add_configs("mem_debug", {
        description = "Enable Memory Debug.",
        default = false,
        type = "boolean"
    })

    add_configs("jemalloc", {
        description = "Enable static linking to the jemalloc library.",
        default = false,
        type = "boolean"
    })
    add_configs("ffmpeg", {
    ...
    add_configs("x264", {
        description = "Enable libx264.",
        default = false,
        type = "boolean"
    })
    add_configs("faac", {
        description = "Enable FAAC.",
        default = false,
        type = "boolean"
    })
    ...
    on_install("macosx", "linux", "windows", function(package)
        ...
        local configs = {
            "-DENABLE_TESTS=OFF", "-DUSE_SOLUTION_FOLDERS=OFF",
            "-DENABLE_SERVER=OFF"
        }
        table.insert(configs, "-DCMAKE_BUILD_TYPE=" ..
                         (package:debug() and "Debug" or "Release"))
        for name, item in pairs(configdeps) do
            table.insert(configs, "-D" .. item .. "=" ..
                             (package:config(name) and "ON" or "OFF"))
        end
        io.replace("CMakeLists.txt", "add_subdirectory(3rdpart)", "",
                   {plain = true})
        io.replace("srt/CMakeLists.txt", "ZLMediaKit::ToolKit", "",
                   {plain = true})
        import("package.tools.cmake").install(package, configs,
                                              {packagedeps = {"zltoolkit"}})
