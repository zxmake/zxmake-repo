package("klib", function()
    ...
    add_configs("url", {
        description = "Enable FILE-like interfaces to libcurl.",
        default = false,
        type = "boolean"
    })
    add_configs("thread", {
        description = "Enable simple multi-threading models.",
        default = false,
        type = "boolean"
    })
    add_configs("open", {
        description = "Enable smart stream opening.",
        default = false,
        type = "boolean"
    })
    ...
    on_install(function(package)
        io.replace("bgzf.c", "fp->uncompressed_block + input_length",
                   "(char*)fp->uncompressed_block + input_length",
                   {plain = true})
        io.replace("bgzf.c", "fp->compressed_block + 18",
                   "(char*)fp->compressed_block + 18", {plain = true})
        io.replace("knetfile.c", "buf + l", "(char*)buf + l", {plain = true})
        if package:is_plat("windows", "mingw") then
            io.replace("khmm.c", "// new/delete hmm_par_t",
                       [[#include "rand48.c"]], {plain = true})
        end
        ...
        if package:is_plat("windows", "mingw") then
            ...
            if package:is_plat("windows") then
                os.cp(path.join(package:scriptdir(), "port", "unistd.h"),
                      "unistd.h")
            end
        end
