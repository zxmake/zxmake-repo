package("krb5", function()

    set_homepage("http://web.mit.edu/kerberos/")
    set_description("Kerberos: The Network Authentication Protocol")

    add_urls("https://kerberos.org/dist/krb5/$(version).tar.gz", {
        version = function(version)
            return format("%d.%d/krb5-%s", version:major(), version:minor(),
                          version)
        end
    })
    add_versions("1.19.2",
    ...
    add_configs("tls", {
        description = "Enable TLS/OpenSSL support.",
        default = false,
        type = "boolean"
    })
    add_configs("shared", {
    ...
    add_links("k5crypto", "kdb5", "krad", "gssapi_krb5", "krb5support", "krb5",
              "gssrpc", "verto", "com_err")
    ...
    on_install("macosx", "linux", function(package)
        ...
        table.insert(configs, "--with-tls-impl=" ..
                         (package:config("tls") and "openssl" or "no"))
        local cppflags = {}
        local ldflags = {}
