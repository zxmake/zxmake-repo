package("keystone", function()
    ...
    add_urls(
        "https://github.com/keystone-engine/keystone/archive/refs/tags/$(version).tar.gz",
    ...
    on_load(function(package)
        if package:is_cross() or package:is_plat("mingw") or
            (package:is_plat("windows") and package:config("shared")) then
    ...
    on_install(function(package)
        io.replace("CMakeLists.txt", "add_subdirectory(suite/fuzz)", "",
                   {plain = true})
        ...
        table.insert(configs, "-DBUILD_LIBS_ONLY=" ..
                         (package:data("build_libs_only") and "ON" or "OFF"))
        if package:is_plat("windows") then
            table.insert(configs, "-DKEYSTONE_BUILD_STATIC_RUNTIME=" ..
        ...
        if package:is_plat("windows") and package:is_debug() then
            ...
            os.trycp(path.join(package:buildir(), "kstool/kstool.pdb"),
    ...
    on_test(function(package)
        ...
        assert(package:has_cfuncs("ks_version",
                                  {includes = "keystone/keystone.h"}))
    end)
end)
