...
on_config(function(target)
    ...
    local config_dir = path.join(target:autogendir(), "rules", "commonlibsse",
                                 "plugin")
    ...
    depend.on_changed(function()
        ...
        if file then
            ...
            file:print("FILEVERSION %s, %s, %s, 0", version:major(),
                       version:minor(), version:patch())
            file:print("PRODUCTVERSION %s, %s, %s, 0", version:major(),
                       version:minor(), version:patch())
            ...
            file:print("        BEGIN")
            file:print("            VALUE \"FileDescription\", \"%s\"",
                       configs.description or "")
            file:print("            VALUE \"FileVersion\", \"%s.0\"",
                       target:version() or "0.0.0")
            file:print("            VALUE \"InternalName\", \"%s\"",
                       configs.name or target:name())
            file:print("            VALUE \"LegalCopyright\", \"%s, %s\"",
                       configs.author or "",
                       target:license() or "Unknown License")
            file:print("            VALUE \"ProductName\", \"%s\"",
                       project.name() or "")
            file:print("            VALUE \"ProductVersion\", \"%s.0\"",
                       project.version() or "0.0.0")
            file:print("        END")
            file:print("    END")
        ...
        dependfile = target:dependfile(version_file),
    ...
    depend.on_changed(function()
        ...
        if file then
            ...
            if configs.options then
                ...
                local signature_scanning =
                    configs.options.signature_scanning or false
            ...
            file:print("    .Version = { %s, %s, %s, 0 },", version:major(),
                       version:minor(), version:patch())
            ...
            file:print(
                "    .StructCompatibility = SKSE::StructCompatibility::%s,",
                struct_compat)
            file:print(
                "    .RuntimeCompatibility = SKSE::VersionIndependence::%s",
                runtime_compat)
            file:print(")")
            file:close()
        end
    end, {
        dependfile = target:dependfile(plugin_file),
        files = project.allfiles()
    })
    ...
    target:add("cxxflags", "/permissive-", "/Zc:alignedNew", "/Zc:__cplusplus",
               "/Zc:forScope", "/Zc:ternary")
    target:add("cxxflags", "cl::/Zc:externConstexpr", "cl::/Zc:hiddenFriend",
               "cl::/Zc:preprocessor", "cl::/Zc:referenceBinding")
end)
