package("cpuinfo", function()
    ...
    add_configs("clog", {
        description = "Build clog library.",
        default = false,
        type = "boolean"
    })
    add_configs("tools", {
    ...
    on_check("windows", function(package)
        ...
        if msvc and package:is_arch("arm.*") then
            ...
            assert(vs_sdkver and semver.match(vs_sdkver):gt("10.0.19041"),
                   "package(cpuinfo): need vs_sdkver > 10.0.19041.0")
        end
    end)
    ...
    on_install("!cross", function(package)
        local configs = {
            "-DCPUINFO_BUILD_TOOLS=OFF", "-DCPUINFO_BUILD_UNIT_TESTS=OFF",
        ...
        table.insert(configs, "-DCPUINFO_LIBRARY_TYPE=" ..
                         (package:config("shared") and "shared" or "static"))
        table.insert(configs, "-DCPUINFO_BUILD_TOOLS=" ..
                         (package:config("tools") and "ON" or "OFF"))

        if package:is_plat("mingw") then
            table.insert(configs, "-DCMAKE_SYSTEM_PROCESSOR=Windows")
        end
        if (package:is_cross() or package:is_plat("mingw")) and
            (not package:is_plat("android")) then
            io.replace("CMakeLists.txt",
                       [[SET(CPUINFO_TARGET_PROCESSOR "${CMAKE_SYSTEM_PROCESSOR}")]],
                       "", {plain = true})
            table.insert(configs,
                         "-DCPUINFO_TARGET_PROCESSOR=" .. package:arch())
        end
