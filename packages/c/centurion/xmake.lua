package("centurion", function()
    ...
    set_urls(
        "https://github.com/albin-johansson/centurion/archive/refs/tags/$(version).tar.gz",
        "https://github.com/albin-johansson/centurion.git")

    add_versions("v7.3.0",
                 "ad8b7c27074939fa46380a878e82c2b0365d1c4ad31b4a71bfcd5ce3ac0198e6")

    add_patches("v7.3.0", path.join(os.scriptdir(), "patches", "7.3.0",
                                    "fix_method_name.patch"),
                "2c5faf24867440e53bdabf79b4354cdbf2c79707ca9e372ba7cdd94bcb1dcc52")

    add_configs("pragma_once", {
        description = "Use #pragma once in centurion.hpp",
        default = true,
        type = "boolean"
    })
    add_configs("debug_macros", {
        description = "Include debug-only logging macros, such as CENTURION_LOG_INFO",
        default = true,
        type = "boolean"
    })

    add_configs("sdl_image", {
        description = "Enable library components that rely on the SDL_image library",
        default = true,
        type = "boolean"
    })
    add_configs("sdl_mixer", {
        description = "Enable library components that rely on the SDL_mixer library",
        default = true,
        type = "boolean"
    })
    add_configs("sdl_ttf", {
        description = "Enable library components that rely on the SDL_ttf library",
        default = true,
        type = "boolean"
    })

    add_configs("vulkan", {
        description = "Enable library components related to Vulkan",
        default = true,
        type = "boolean"
    })
    add_configs("opengl", {
        description = "Enable library components related to OpenGL",
        default = true,
        type = "boolean"
    })
    ...
    on_test(function(package)
        assert(package:check_cxxsnippets({
            ...
            configs = {languages = "c++17", defines = "SDL_MAIN_HANDLED"},
            includes = "centurion.hpp"
        }))
        if package:config("sdl_image") then
            assert(package:check_cxxsnippets({
                test = [[
