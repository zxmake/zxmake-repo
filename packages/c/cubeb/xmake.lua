package("cubeb", function()
    ...
    add_configs("speex", {
        description = "Bundle the speex library",
        default = false,
        type = "boolean"
    })
    add_configs("lazy_load", {
        description = "Lazily load shared libraries",
        default = true,
        type = "boolean"
    })
    ...
    on_install("windows", "linux", "macosx", "bsd", "mingw", "msys", "android",
        ...
        local configs = {
            "-DBUILD_TESTS=OFF", "-DBUILD_RUST_LIBS=OFF", "-DBUILD_TOOLS=OFF",
            "-DUSE_SANITIZERS=OFF"
        }
        ...
        table.insert(configs, "-DBUNDLE_SPEEX=" ..
                         (package:config("speex") and "ON" or "OFF"))
        table.insert(configs, "-DLAZY_LOAD_LIBS=" ..
                         (package:config("lazy_load") and "ON" or "OFF"))
        import("package.tools.cmake").install(package, configs)
    end)

    on_test(function(package)
        assert(package:has_cxxfuncs("cubeb_init", {
            includes = "cubeb/cubeb.h",
