package("civetweb", function()
    ...
    add_urls(
        "https://github.com/civetweb/civetweb/archive/refs/tags/$(version).tar.gz",
        "https://github.com/civetweb/civetweb.git")
    add_versions('v1.15',
                 '90a533422944ab327a4fbb9969f0845d0dba05354f9cacce3a5005fa59f593b9')
    add_patches("v1.15", path.join(os.scriptdir(), "patches", "v1.15",
                                   "find-winsock.patch"),
                "4d8355169fb20ede0fdae2a8bc8561238a088cf810e36107ba267f273989d9ee")
    ...
    on_load(function(package)
        local configdeps = {openssl = "openssl", zlib = "zlib"}
    ...
    on_install(function(package)
        local configs = {
            "-DCIVETWEB_BUILD_TESTING=OFF", "-DCIVETWEB_ENABLE_DEBUG_TOOLS=OFF",
        ...
        table.insert(configs, "-DCIVETWEB_ENABLE_SSL=" ..
                         (package:config("openssl") and "ON" or "OFF"))
        table.insert(configs, "-DCIVETWEB_ENABLE_ZLIB=" ..
        ...
        if package:is_plat("mingw") and package:config("shared") then
            io.replace("src/CMakeLists.txt", "if (WINSOCK_FOUND)", "if (TRUE)",
                       {plain = true})
            io.replace("src/CMakeLists.txt", "WINSOCK::WINSOCK", "ws2_32",
                       {plain = true})
        end
