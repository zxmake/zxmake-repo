package("cairo", function()
    ...
    add_versions("1.17.6",
                 "a2227afc15e616657341c42af9830c937c3a6bfa63661074eabef13600e8936f")
    add_versions("1.17.8",
                 "b4ed6d33037171d4c6594345b42d81796f335a6995fdf5638db0d306c17a0d3e")
    add_versions("1.18.0",
                 "39a78afdc33a435c0f2ab53a5ec2a693c3c9b6d2ec9783ceecb2b94d54d942b0")

    add_patches("1.18.0",
                path.join(os.scriptdir(), "patches", "1.18.0", "alloca.patch"),
                "55f8577929537d43eed9f74241560821001b6c8613d6a7a21cff83f8431c6a70")
    ...
    add_configs("fontconfig", {
        description = "Enable fontconfig support.",
        default = true,
        type = "boolean"
    })
    add_configs("xlib", {
        description = "Enable x11 surface backend.",
        default = is_plat("linux"),
        type = "boolean"
    })
    add_configs("glib", {
        description = "Enable glib dependency.",
        default = false,
        type = "boolean"
    })
    ...
    elseif is_plat("macosx") then
        add_frameworks("CoreGraphics", "CoreFoundation", "CoreText",
                       "Foundation")
    end
    ...
    on_install("windows|x64", "windows|x86", "macosx", "linux",
        ...
        local configs = {
            "--wrap-mode=nopromote", "-Dtests=disabled", "-Dgtk_doc=false",
            "-Dgtk2-utils=disabled", "-Dpng=enabled", "-Dzlib=enabled"
        }
        ...
        table.insert(configs, "-Dfontconfig=" ..
                         (package:config("fontconfig") and "enabled" or
                             "disabled"))
        table.insert(configs, "-Dxlib=" ..
                         (package:config("xlib") and "enabled" or "disabled"))
        ...
        io.replace("meson.build", "'CoreFoundation'",
                   "'CoreFoundation', 'Foundation'", {plain = true})
        local envs = meson.buildenvs(package, {
            packagedeps = {"libintl", "libiconv", "lzo"}
        })
        if package:is_plat("windows") then
            envs.PATH = package:dep("pkgconf"):installdir("bin") ..
