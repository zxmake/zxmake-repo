package("charls", function()
    ...
    on_install(function(package)
        local configs = {
            "-DCHARLS_BUILD_TESTS=OFF", "-DCHARLS_BUILD_AFL_FUZZ_TEST=OFF",
            "-DCHARLS_BUILD_LIBFUZZER_FUZZ_TEST=OFF",
            "-DCHARLS_BUILD_SAMPLES=OFF", "-DCHARLS_INSTALL=ON"
        }
    ...
    on_test(function(package)
        assert(package:has_cfuncs("charls_jpegls_encoder_create",
                                  {includes = "charls/charls.h"}))
    end)
end)
