package("cserialport", function()
    ...
    add_urls(
        "https://github.com/itas109/CSerialPort/archive/refs/tags/$(version).tar.gz",
        "https://github.com/itas109/CSerialPort.git")

    add_versions("v4.3.1",
                 "376f41866be65ddfed91f3d0fea91aaaf5ca7e645f9b9cfcdaa0a9182a0bb3ac")
        ...
        on_check("windows", function(package)
            ...
            if package:is_arch("arm.*") then
                ...
                assert(vs_toolset and semver.new(vs_toolset):minor() >= 30,
                       "package(cserialport/arm): need vs_toolset >= v143")
            end
        end)
    end
    ...
    on_install("!cross and !iphoneos and !android", function(package)
        ...
        if package:config("c_api") then
            io.replace("bindings/c/cserialport.h",
                       "#define C_DLL_EXPORT __declspec(dllexport)",
                       "#define C_DLL_EXPORT", {plain = true})
    ...
    on_test(function(package)
        assert(package:check_cxxsnippets({
            ...
            includes = "CSerialPort/SerialPort.h"
        }))

        if package:config("c_api") then
            assert(package:has_cfuncs("CSerialPortInit",
                                      {includes = "cserialport.h"}))
        end
    end)
end)
