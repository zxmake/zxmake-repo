package("cuda", function()
    ...
    add_configs("utils", {
        description = "Enabled cuda utilities.",
        default = {},
        type = "table"
    })
    ...
    on_fetch(function(package, opt)
        if opt.system then
            ...
            if cuda then
                local result = {
                    includedirs = cuda.includedirs,
                    linkdirs = cuda.linkdirs,
                    links = {}
                }
                local utils = package:config("utils")
                table.insert(utils, package:config("shared") and "cudart" or
                                 "cudart_static")
