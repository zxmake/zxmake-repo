package("cnats", function()
    set_homepage("https://github.com/nats-io/nats.c")
    set_description("A C client for NATS")
    set_license("Apache-2.0")

    add_urls(
        "https://github.com/nats-io/nats.c/archive/refs/tags/$(version).tar.gz",
        "https://github.com/nats-io/nats.c.git")

    add_versions("v3.8.2",
                 "083ee03cf5a413629d56272e88ad3229720c5006c286e8180c9e5b745c10f37d")
    ...
    add_configs("sodium", {
        description = "Build with libsodium",
        default = false,
        type = "boolean"
    })
    add_configs("streaming", {
        description = "Build NATS Streaming",
        default = false,
        type = "boolean"
    })
    ...
    on_install("!bsd", function(package)
        ...
        table.insert(configs, "-DNATS_BUILD_LIB_SHARED=" ..
                         (package:config("shared") and "ON" or "OFF"))
        table.insert(configs, "-DNATS_BUILD_LIB_STATIC=" ..
                         (package:config("shared") and "OFF" or "ON"))

        table.insert(configs, "-DNATS_BUILD_STREAMING=" ..
                         (package:config("streaming") and "ON" or "OFF"))
        table.insert(configs, "-DNATS_BUILD_TLS_USE_OPENSSL_1_1_API=ON")
        table.insert(configs, "-DNATS_BUILD_WITH_TLS=" ..
                         (package:config("tls") and "ON" or "OFF"))
        table.insert(configs, "-DNATS_BUILD_USE_SODIUM=" ..
                         (package:config("sodium") and "ON" or "OFF"))
        ...
        if package:config("sodium") then
            ...
            if not libsodium:is_system() then
                table.insert(configs,
                             "-DNATS_SODIUM_DIR=" .. libsodium:installdir())
                io.replace("CMakeLists.txt", "libsodium.lib libsodium.dll",
                           "sodium.lib sodium.dll", {plain = true})
            end
