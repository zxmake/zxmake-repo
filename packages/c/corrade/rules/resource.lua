...
on_buildcmd_file(function(target, batchcmds, sourcefile, opt)
    ...
    batchcmds:show_progress(opt.progress,
                            "${color.build.object}compiling.corrade %s",
                            sourcefile)

    -- get corrade-rc program
    local corrade = find_program("corrade-rc", {check = "-h"})
    assert(corrade,
           "corrade-rc not found! please check your corrade installation.")

    -- generate source file
    local basename = path.basename(sourcefile)
    local sourcefile_cx = path.join(target:autogendir(), "rules", "autogen",
                                    basename .. ".cpp")
