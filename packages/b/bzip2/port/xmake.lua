...
if has_config("enable_tools") then
    ...
    after_install(function(target)
        ...
        if target:is_plat("windows", "mingw") then
            ...
            os.vcp(path.join(binarydir, "bzip2.exe"),
                   path.join(binarydir, "bzcat.exe"))
            os.vcp(path.join(binarydir, "bzip2.exe"),
                   path.join(binarydir, "bunzip2.exe"))
        elseif is_subhost("windows") then
            ...
            os.vcp(path.join(binarydir, "bzip2"),
                   path.join(binarydir, "bunzip2"))
        else
            local binarydir = path.join(target:installdir(), "bin")
            ...
            os.ln(path.join(binarydir, "bzgrep"),
                  path.join(binarydir, "bzegrep"))
            os.ln(path.join(binarydir, "bzgrep"),
                  path.join(binarydir, "bzfgrep"))
