...
on_buildcmd_file(function(target, batchcmds, shaderfile, opt)
    ...
    batchcmds:show_progress(opt.progress,
                            "${color.build.object}compiling.shaderc %s",
                            shaderfile)

    -- get bgfx shaderc
    local shaderc = find_program("shadercRelease") or
                        find_program("shadercDebug")
    assert(shaderc,
           "bgfx shaderc not found! please check your bgfx installation.")
    ...
    if fileconfig and fileconfig.vardef then
        ...
        vardef_filename =
            path.join(path.directory(shaderfile), "varying.def.sc")
    end
    ...
    local bgfx_types = {"vertex", "fragment", "compute"}
    ...
    elseif shaderfile:match("^cs_.*%.sc$") or shaderfile:match("%.comp$") then
        ...
        raise("cannot determine shader type from file name " ..
                  path.filename(shaderfile))
    end

    -- build command args
    local args = {
        "-f", shaderfile, "--type", shader_type, "--varyingdef",
        vardef_filename, "--platform", bgfx_platforms[target:plat()]
    }
