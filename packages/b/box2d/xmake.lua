package("box2d", function()
    set_homepage("https://box2d.org")
    set_description("A 2D Physics Engine for Games")
    set_license("MIT")

    set_urls(
        "https://github.com/erincatto/box2d/archive/refs/tags/$(version).tar.gz",
        "https://github.com/erincatto/box2d.git")

    add_versions("v3.0.0",
                 "64ad759006cd2377c99367f51fb36942b57f0e9ad690ed41548dd620e6f6c8b1")
    add_versions("v2.4.2",
        ...
        on_check(function(package)
            if package:version():ge("3.0.0") then
                ...
                if package:is_plat("windows") then
                    assert(not package:is_arch("arm64.*"),
                           "package(box2d/arm >=3.0.0) Unsupported architecture.")
                end
                ...
                if package:is_plat("android") then
                    ...
                    assert(ndk_sdkver and tonumber(ndk_sdkver) >= 28,
                           "package(box2d >=3.0.0) requires ndk api level >= 28")
                end
                ...
                assert(
                    package:has_cincludes("stdatomic.h", {configs = configs}),
    ...
    on_install("!bsd", function(package)
        ...
        local configs = {
            ...
            "-DBOX2D_VALIDATE=OFF", "--compile-no-warning-as-error"
        }
        ...
        table.insert(configs, "-DBOX2D_SANITIZE=" ..
                         (package:config("asan") and "ON" or "OFF"))
        table.insert(configs, "-DBOX2D_AVX2=" ..
        ...
        if package:config("shared") then
            os.trycp(path.join(package:buildir(), "bin/box2d.pdb"),
