package("binutils", function()
    ...
    on_install("@linux", "@macosx", "@msys", function(package)
        local configs = {
            ...
            "--mandir=" .. package:installdir("share/man"), "--disable-werror",
            "--enable-interwork", "--enable-multilib", "--enable-64-bit-bfd",
            "--enable-targets=all"
        }
        ...
        if package:version():le("2.34") then
            ...
            io.replace("binutils/srconv.c", "char *program_name;",
                       "extern char *program_name;", {plain = true})
            io.replace("binutils/sysdump.c", "char *program_name;",
                       "extern char *program_name;", {plain = true})
            io.replace("binutils/coffdump.c", "char * program_name;",
                       "extern char *program_name;", {plain = true})
        end
