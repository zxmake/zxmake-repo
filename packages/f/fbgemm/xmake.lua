package("fbgemm", function()
    ...
    add_urls(
        "https://github.com/pytorch/FBGEMM/archive/refs/tags/$(version).tar.gz",
        "https://github.com/pytorch/FBGEMM.git")

    add_versions("v0.8.0",
                 "f754dbc6becf8ece0474872c4e797445b55c21799c1f1d219470c0c5818207dd")
    add_versions("v0.7.0",
    ...
    if is_plat("windows") then
        add_patches("0.8.0", "patches/0.8.0/msvc-omp.patch",
                    "d4a7830e40a476ffdeda00d2f7901a7db6e7950392ff672144d5e9f3c37ced2f")
    end

    -- need libtorch
    add_configs("gpu", {
        description = "Build fbgemm_gpu library",
        default = false,
        type = "boolean"
    })
    add_configs("cpu", {
        description = "Build FBGEMM_GPU without GPU support",
        default = false,
        type = "boolean"
    })
    add_configs("rocm", {
        description = "Build FBGEMM_GPU for ROCm",
        default = false,
        type = "boolean"
    })
    ...
    on_install("windows|x64", "linux|x86_64", "macosx|x86_64", function(package)
        ...
        local configs = {
            "-DFBGEMM_BUILD_TESTS=OFF", "-DFBGEMM_BUILD_BENCHMARKS=OFF"
        }
        table.insert(configs, "-DCMAKE_BUILD_TYPE=" ..
                         (package:is_debug() and "Debug" or "Release"))
        table.insert(configs, "-DFBGEMM_LIBRARY_TYPE=" ..
                         (package:config("shared") and "shared" or "static"))

        table.insert(configs, "-DFBGEMM_BUILD_FBGEMM_GPU=" ..
                         (package:config("gpu") and "ON" or "OFF"))
        table.insert(configs, "-DFBGEMM_CPU_ONLY=" ..
                         (package:config("cpu") and "ON" or "OFF"))
        table.insert(configs, "-DUSE_ROCM=" ..
                         (package:config("rocm") and "ON" or "OFF"))
        import("package.tools.cmake").install(package, configs, {
            packagedeps = {"asmjit", "cpuinfo"}
        })
    end)
    ...
    on_test(function(package)
        assert(package:check_cxxsnippets({
            ...
            includes = "fbgemm/QuantUtilsAvx2.h"
        }))
    end)
end)
