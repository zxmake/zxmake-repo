package("fontconfig", function()
    ...
    add_versions("2.13.93",
                 "0f302a18ee52dde0793fe38b266bf269dfe6e0c0ae140e30d72c6cca5dc08db5")
    ...
    add_patches("2.13.1",
                "https://gitlab.freedesktop.org/fontconfig/fontconfig/commit/8208f99fa1676c42bfd8d74de3e9dac5366c150c.diff",
                "2abdff214b99f2d074170e6512b0149cc858ea26cd930690aa6b4ccea2c549ef")

    add_configs("nls", {
        description = "Enable Native Language Support (NLS)",
        default = false,
        type = "boolean"
    })
    ...
    on_install("windows", "linux", "macosx", function(package)
        if package:is_plat("windows") then
            io.replace("meson.build", "c_args = []",
                       "c_args = ['-DXML_STATIC']", {plain = true})
        end
        local configs = {
            "-Dtests=disabled", "-Dtools=disabled", "-Ddoc=disabled"
        }
        table.insert(configs,
                     "-Ddebug=" .. (package:debug() and "true" or "false"))
        table.insert(configs, "-Ddefault_library=" ..
                         (package:config("shared") and "shared" or "static"))
        table.insert(configs, "-Dnls=" ..
                         (package:config("nls") and "enabled" or "disabled"))
        import("package.tools.meson").install(package, configs)
    end)

    on_test(function(package)
        assert(package:has_cfuncs("FcInitLoadConfigAndFonts",
                                  {includes = "fontconfig/fontconfig.h"}))
    end)
end)
