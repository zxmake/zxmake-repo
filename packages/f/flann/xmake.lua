package("flann", function()
    ...
    add_patches("1.9.1",
                path.join(os.scriptdir(), "patches", "1.9.1", "cuda10.patch"),
                "087492b422362f42c83b320296d9af7b22023cf20d81ea7bd99efabd1535f7d6")

    add_configs("with_cuda", {
    ...
    on_load(function(package)
        ...
        local libs = package:config("with_cuda") and {"flann"} or
                         {"flann", "flann_cuda"}
    ...
    on_install(function(package)
        os.cd("src/cpp")
        io.replace("flann/util/serialization.h", "flann/ext/lz4", "lz4",
                   {plain = true})
        io.replace("flann/defines.h", "#ifdef WIN32", "#ifdef _WIN32",
                   {plain = true})
