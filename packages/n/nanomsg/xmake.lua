package("nanomsg", function()
    set_homepage("https://nanomsg.org")
    set_description(
        [[A simple high-performance implementation of several "scalability protocols".]])

    add_urls(
        "https://github.com/nanomsg/nanomsg/archive/refs/tags/$(version).tar.gz",
        "https://github.com/nanomsg/nanomsg.git")

    add_versions("1.2.1",
                 "2e6c20dbfcd4882e133c819ac77501e9b323cb17ae5b3376702c4446261fbc23")
    ...
    on_install(function(package)
        ...
        if package:has_tool("cc", "gcc") then
            ...
            io.replace("src/aio/usock_win.inc",
                       "self->in.start = nn_usock_recv_start_pipe;",
                       "nn_usock_recv_start_pipe(self->in.arg);", {plain = true})
            io.replace("src/aio/usock_win.inc",
                       "self->in.start = nn_usock_recv_start_wsock;",
                       "nn_usock_recv_start_wsock(self->in.arg);",
                       {plain = true})
            io.replace("src/aio/usock_win.inc",
                       "self->in.start (self->in.arg);", "", {plain = true})
        end

        local configs = {
            "-DNN_ENABLE_DOC=OFF", "-DNN_TESTS=OFF",
            "-DCMAKE_POLICY_DEFAULT_CMP0057=NEW"
        }
        table.insert(configs, "-DCMAKE_BUILD_TYPE=" ..
                         (package:is_debug() and "Debug" or "Release"))
        table.insert(configs, "-DNN_STATIC_LIB=" ..
                         (package:config("shared") and "OFF" or "ON"))
        table.insert(configs, "-DNN_TOOLS=" ..
