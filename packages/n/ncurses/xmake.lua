package("ncurses", function()
    ...
    add_configs("widec", {
        description = "Compile with wide-char/UTF-8 code.",
        default = true,
        type = "boolean"
    })
    ...
    on_install("linux", "macosx", "bsd", "msys", function(package)
        local configs = {
            "--without-manpages", "--enable-sigwinch", "--with-gpm=no",
            "--without-tests", "--without-ada"
        }

        table.insert(configs,
                     "--with-debug=" .. (package:is_debug() and "yes" or "no"))
        table.insert(configs, "--with-shared=" ..
                         (package:config("shared") and "yes" or "no"))
        table.insert(configs, "--enable-widec=" ..
                         (package:config("widec") and "yes" or "no"))
