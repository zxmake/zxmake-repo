package("nanogui", function()
    ...
    if is_plat("macosx") then
        add_frameworks("CoreFoundation", "CoreGraphics", "CoreVideo", "IOKit",
                       "AppKit")
    ...
    on_install("windows", "macosx", "linux", function(package)
        local configs = {
            "-DNANOGUI_BUILD_EXAMPLE=OFF", "-DNANOGUI_BUILD_PYTHON=OFF"
        }
        table.insert(configs, "-DCMAKE_BUILD_TYPE=" ..
                         (package:debug() and "Debug" or "Release"))
        table.insert(configs, "-DNANOGUI_BUILD_SHARED=" ..
                         (package:config("shared") and "ON" or "OFF"))
        table.insert(configs, "-DBUILD_SHARED_LIBS=" ..
                         (package:config("shared") and "ON" or "OFF"))
        io.replace("CMakeLists.txt", '$<TARGET_OBJECTS:glfw_objects>', "",
                   {plain = true})
        io.replace("CMakeLists.txt", 'ext/nanovg/src/nanovg.c', "",
                   {plain = true})
        ...
        if package:is_plat("linux", "windows") then
            io.replace("include/nanogui/opengl.h", '#include <nanovg.h>',
                       [[#include <nanovg.h>
#ifdef _MSC_VER
#   include <windows.h>
...
#include <GL/gl.h>]], {plain = true})
        ...
        if package:is_plat("linux") then
            table.join2(packagedeps, "libxi", "libxcursor", "libxinerama",
                        "libxrandr", "libxxf86vm")
            io.replace("CMakeLists.txt",
                       'target_link_libraries(nanogui ${NANOGUI_EXTRA_LIBS})',
                       'target_link_libraries(nanogui ${NANOGUI_EXTRA_LIBS} nanovg)',
                       {plain = true})
        end
