package("nasm", function()
    ...
    if is_host("windows") then
        if os.arch() == "x64" then
            add_urls(
                "https://www.nasm.us/pub/nasm/releasebuilds/$(version)/win64/nasm-$(version)-win64.zip")
            add_urls(
                "https://github.com/xmake-mirror/nasm/releases/download/nasm-$(version)/nasm-$(version)-win64.zip")
            add_versions("2.15.05",
                         "f5c93c146f52b4f1664fa3ce6579f961a910e869ab0dae431bd871bdd2584ef2")
            add_versions("2.16.01",
                         "029eed31faf0d2c5f95783294432cbea6c15bf633430f254bb3c1f195c67ca3a")
        else
            add_urls(
                "https://www.nasm.us/pub/nasm/releasebuilds/$(version)/win32/nasm-$(version)-win32.zip")
            add_urls(
                "https://github.com/xmake-mirror/nasm/releases/download/nasm-$(version)/nasm-$(version)-win32.zip")
            add_versions("2.15.05",
                         "258c7d1076e435511cf2fdf94e2281eadbdb9e3003fd57f356f446e2bce3119e")
            add_versions("2.16.01",
                         "e289fa70c88594b092c916344bb8bfcd6896b604bfab284ab57b1372997c820c")
        end
    else
        add_urls(
            "https://www.nasm.us/pub/nasm/releasebuilds/$(version)/nasm-$(version).tar.xz")
        add_urls(
            "https://github.com/xmake-mirror/nasm/releases/download/nasm-$(version)/nasm-$(version).tar.xz")
        add_versions("2.13.03",
