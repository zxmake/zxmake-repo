package("nlopt", function()
    ...
    add_urls(
        "https://github.com/stevengj/nlopt/archive/refs/tags/$(version).tar.gz",
    ...
    add_configs("octave", {
        description = "build octave bindings",
        default = false,
        type = "boolean",
        readonly = true
    })
    add_configs("matlab", {
        description = "build matlab bindings",
        default = false,
        type = "boolean",
        readonly = true
    })
    add_configs("guile", {
        description = "build guile bindings",
        default = false,
        type = "boolean",
        readonly = true
    })
    ...
    on_install(function(package)
        local configs = {
            "-DNLOPT_TESTS=OFF", "-DNLOPT_PYTHON=OFF", "-DNLOPT_SWIG=OFF"
        }
        ...
        table.insert(configs, "-DNLOPT_OCTAVE=" ..
                         (package:config("octave") and "ON" or "OFF"))
        table.insert(configs, "-DNLOPT_MATLAB=" ..
                         (package:config("matlab") and "ON" or "OFF"))
        table.insert(configs, "-DNLOPT_GUILE=" ..
                         (package:config("guile") and "ON" or "OFF"))
