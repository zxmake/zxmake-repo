package("openxlsx", function()
    ...
    add_configs("compact_mode", {
        description = "Build library in compact mode (slower, but uses less memory)",
        default = false,
        type = "boolean"
    })
    ...
    on_install(function(package)
        ...
        if package:is_plat("mingw") then
            io.replace("OpenXLSX/sources/XLDocument.cpp",
                       "#    define stat _stat", "", {plain = true})
        end

        local configs = {
            "-DOPENXLSX_CREATE_DOCS=OFF", "-DOPENXLSX_BUILD_SAMPLES=OFF",
            "-DOPENXLSX_BUILD_TESTS=OFF", "-DOPENXLSX_BUILD_BENCHMARKS=OFF"
        }
        table.insert(configs, "-DCMAKE_BUILD_TYPE=" ..
                         (package:is_debug() and "Debug" or "Release"))
        table.insert(configs, "-DOPENXLSX_LIBRARY_TYPE=" ..
                         (package:config("shared") and "SHARED" or "STATIC"))
        table.insert(configs, "-DOPENXLSX_COMPACT_MODE=" ..
                         (package:config("compact_mode") and "ON" or "OFF"))
