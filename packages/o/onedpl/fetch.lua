...
function _find_package(package, opt)
    local paths = {"$(env DPL_ROOT)", "$(env ONEAPI_ROOT)/dpl/latest"}
    -- find library
    local result = {links = {}, linkdirs = {}, includedirs = {}}

    -- find include
    local includepath = find_path(path.join("oneapi", "dpl", "algorithm"),
                                  paths, {suffixes = "include"})
    if includepath then
        table.insert(result.includedirs, includepath)
    end

    if #result.includedirs > 0 then
        local version_file = path.join(includepath, "oneapi", "dpl", "pstl",
                                       "onedpl_config.h")
