package("openh264", function()
    ...
    set_urls(
        "https://github.com/cisco/openh264/archive/refs/tags/$(version).tar.gz",
        "https://github.com/cisco/openh264.git")

    add_versions("v2.4.1",
                 "8ffbe944e74043d0d3fb53d4a2a14c94de71f58dbea6a06d0dc92369542958ea")
    add_versions("v2.1.1",
                 "af173e90fce65f80722fa894e1af0d6b07572292e76de7b65273df4c0a8be678")
    ...
    on_install("!android and !bsd and !wasm", function(package)
        if package:gitref() or package:version():ge("2.4.1") then
            ...
            if package:is_plat("windows") and package:is_arch("arm.*") then
                opt.envs.PATH = path.join(os.programdir(), "scripts") ..
                                    path.envsep() .. opt.envs.PATH
            end
            ...
            import("package.tools.meson").build(package, {"-Dtests=disabled"},
                                                {buildir = "out"})
            ...
            if package:config("shared") then
                ...
                os.tryrm(path.join(package:installdir("lib"),
                                   "libopenh264.dylib"))
                os.tryrm(path.join(package:installdir("lib"), "openh264.lib"))
                os.tryrm(path.join(package:installdir("bin"), "openh264-*.dll"))
            end
            if package:is_plat("windows") then
                os.trymv(path.join(package:installdir("lib"), "libopenh264.a"),
                         path.join(package:installdir("lib"), "openh264.lib"))
            end
        end
    end)

    on_test(function(package)
        assert(package:has_cxxfuncs("WelsGetCodecVersion",
                                    {includes = "wels/codec_api.h"}))
    end)
end)
