package("opencv-mobile", function()
    ...
    local version_map = {["4.10.0"] = "v29", ["3.4.20"] = "v29"}
    add_urls(
        "https://github.com/nihui/opencv-mobile/releases/download/$(version).zip",
        {
            version = function(version)
                ...
                return
                    string.format("%s/opencv-mobile-%s", v, tostring(version))
            end
        })

    add_versions("4.10.0",
                 "e9209285ad4d682536db4505bc06e46b94b9e56d91896e16c2853c83a870f004")
    add_versions("3.4.20",
    ...
    on_install("linux", "macosx", "windows", "mingw@windows,msys",
        ...
        table.remove_if(options, function(_, option)
            return option:startswith("-DCMAKE_BUILD_TYPE") or
                       option:startswith("-DBUILD_SHARED_LIBS") or
                       option:startswith("-DBUILD_WITH_STATIC_CRT")
        end)
        ...
        if package:is_plat("windows") then
            ...
            table.join2(libfiles, os.files(
                            package:installdir(arch, vc_ver, linkdir, "*.lib")))
        ...
        elseif package:is_plat("mingw") then
            local arch = package:is_arch("x86_64") and "x64" or "x86"
            local linkdir = (package:config("shared") and "lib" or "staticlib")
            for _, f in ipairs(os.files(package:installdir(arch, "mingw",
                                                           linkdir, "lib*.a"))) do
    ...
    on_test(function(package)
        assert(package:check_cxxsnippets({
            ...
            includes = package:version():ge("3.0") and "opencv2/opencv.hpp" or
                "opencv/cv.h"
        }))
    end)
end)
