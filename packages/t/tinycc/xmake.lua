package("tinycc", function()
    ...
    if is_host("windows") then
        if os.arch() == "x86" then
            set_urls(
                "http://download.savannah.gnu.org/releases/tinycc/tcc-$(version)-win32-bin.zip")
            add_versions("0.9.27",
                         "02e2bfe8c272a549b15e4bfa4507bd7e05304692af1761db6c1e8e88af675651")
        else
            set_urls(
                "http://download.savannah.gnu.org/releases/tinycc/tcc-$(version)-win64-bin.zip")
            add_versions("0.9.27",
