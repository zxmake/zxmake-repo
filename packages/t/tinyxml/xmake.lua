package("tinyxml", function()
    ...
    add_urls(
        "https://sourceforge.net/projects/tinyxml/files/tinyxml/$(version).zip",
        {
            version = function(version)
                return version .. "/tinyxml_" .. version:gsub("%.", "_")
            end
        })
    add_versions("2.6.2",
    ...
    add_configs("stl", {
        description = "Enable STL support.",
        default = true,
        type = "boolean"
    })
    ...
    on_install(function(package)
        if package:config("stl") then
            io.replace("tinyxml.h", "#define TINYXML_INCLUDED",
