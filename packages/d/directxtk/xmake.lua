package("directxtk", function()
    set_homepage("https://github.com/microsoft/DirectXTK")
    set_description(
        "A collection of helper classes for writing Direct3D 11 C++ code For Windows.")
    ...
    add_versions("20.9.0",
                 "9d5131243bf3e33db2e3a968720d860abdcbbe7cb037c2cb5dd06046d439ed09")
    add_versions("21.4.0",
                 "481e769b1aabd08b46659bbec8363a2429f04d3bb9a1e857eb0ebd163304d1bf")
    add_versions("21.11.0",
                 "d25e634b0e225ae572f82d0d27c97051b0069c6813d7be12453039a504dffeb8")
    add_versions("24.2.0",
        ...
        on_check("windows", function(package)
            ...
            if vs_sdkver then
                local build_ver = string.match(vs_sdkver,
                                               "%d+%.%d+%.(%d+)%.?%d*")
                assert(tonumber(build_ver) >= 19041,
                       "DirectXTK requires Windows SDK to be at least 10.0.19041.0")
            end
        end)
    end
    ...
    on_install("windows", function(package)
        ...
        io.replace("Src/Shaders/CompileShaders.cmd", " %1.hlsl ",
                   " \"%1.hlsl\" ", {plain = true})
        io.replace("Src/Shaders/CompileShaders.cmd", " %1.fx ", " \"%1.fx\" ",
                   {plain = true})
    ...
    on_test(function(package)
        assert(package:check_cxxsnippets({
            ...
            includes = {"windows.h", "SimpleMath.h"}
        }))
    end)
end)
