package("m4", function()
    ...
    if is_host("macosx") then
        ...
        add_patches("1.4.18", path.join(os.scriptdir(), "patches", "1.4.18",
                                        "secure_snprintf.patch"),
                    "c0a408fbffb7255fcc75e26bd8edab116fc81d216bfd18b473668b7739a4158e")
    elseif is_host("linux") then
        add_extsources("apt::m4", "pacman::m4")
    end

    on_install("@macosx", "@linux", "@msys", "@cygwin", "@bsd",
               function(package)
        if package:is_plat("linux") then
            ...
            for _, gnulib_file in ipairs(os.files("lib/*.c")) do
                io.replace(gnulib_file, "defined _IO_ftrylockfile",
                           "defined _IO_EOF_SEEN || defined _IO_ftrylockfile")
            end
