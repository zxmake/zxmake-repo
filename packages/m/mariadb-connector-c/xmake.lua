package("mariadb-connector-c", function()
    ...
    add_urls(
        "https://github.com/mariadb-corporation/mariadb-connector-c/archive/refs/tags/v$(version).tar.gz")

    add_versions("3.3.9",
                 "062b9ec5c26cbb236a78f0ba26981272053f59bdfc113040bab904a9da36d31f")
    add_versions("3.3.4",
                 "ea6a23850d6a2f6f2e0d9e9fdb7d94fe905a4317f73842272cf121ed25903e1f")
    add_versions("3.1.13",
                 "361136e9c365259397190109d50f8b6a65c628177792273b4acdb6978942b5e7")
    ...
    add_configs("ssl", {
        ...
        default = not is_plat("bsd"),
        type = "boolean",
        readonly = is_plat("bsd")
    })
    add_configs("dyncol", {
    ...
    on_load(function(package)
        ...
        local configdeps = {external_zlib = "zlib", ssl = "openssl"}
    ...
    on_install("bsd", "linux", "windows", function(package)
        ...
        if package:is_plat("windows") then
            for _, lib in ipairs(os.files(
                                     path.join(package:installdir("lib"),
                                               "mariadb", "*.lib"))) do
                os.trycp(lib, path.join(package:installdir("lib"),
                                        path.filename(lib)))
            end
        else
            for _, lib in ipairs(os.files(
        ...
        if package:config("shared") then
            os.trycp(path.join(package:installdir("lib"), "mariadb", "*.dll"),
                     package:installdir("bin"))
            os.trycp(path.join(package:installdir("lib"), "mariadb", "*.so"),
                     package:installdir("bin"))
            os.cp(path.join(package:installdir("lib"), "mariadb", "plugin"),
                  package:installdir("bin"))
        end
    end)
    ...
    on_test(function(package)
        assert(
            package:has_cxxfuncs("mysql_init", {includes = "mariadb/mysql.h"}))
    end)
end)
