package("mmloader", function()
    ...
    add_configs("shellcode", {
        description = "Generate the shellcode header files",
        default = false,
        type = "boolean"
    })
    add_configs("shared", {
    ...
    if on_check then
        on_check(function(package)
            ...
            assert(vs_sdkver and not semver.match(vs_sdkver):eq("10.0.19041"),
                   "package(mmloader) require vs_sdkver != 10.0.19041.0")
        end)
    end
    ...
    on_install("windows|x64", "windows|x86", function(package)
        ...
        table.insert(configs, "-DBUILD_SHELLCODE_GEN=" ..
                         (package:config("shellcode") and "ON" or "OFF"))
        ...
        if package:is_debug() then
            opt.cxflags = "-D_DEBUG"
            os.mkdir(path.join(package:buildir(),
                               "tools/shellcode-generator/pdb"))
        end
    ...
    on_test(function(package)
        assert(package:has_cfuncs("LoadMemModule",
                                  {includes = "mmLoader/mmLoader.h"}))
    end)
end)
