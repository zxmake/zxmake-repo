package("mcfgthread", function()
    ...
    add_urls(
        "https://github.com/lhmouse/mcfgthread/archive/refs/tags/$(version).tar.gz",
        {
            version = function(version)
                return format("v%d.%d-ga.%d", version:major(), version:minor(),
                              version:patch())
            end
        })
    ...
    on_install("mingw", "msys", function(package)
        ...
        table.insert(configs, "-Denable-debug-checks=" ..
                         (package:config("debug_checks") and "true" or "false"))
        import("package.tools.meson").install(package, configs)
    end)

    on_test(function(package)
        assert(package:has_cfuncs("_MCF_utc_now",
                                  {includes = "mcfgthread/clock.h"}))
    end)
end)
