package("minifb", function()
    ...
    if is_plat("macosx") then
        add_frameworks("CoreFoundation", "Foundation", "AppKit", "MetalKit",
                       "Metal")
    ...
    on_install("macosx", "linux", "windows", function(package)
        ...
        if package:is_plat("linux") then
            ...
            io.replace("CMakeLists.txt", 'set(CMAKE_C_FLAGS "")', "",
                       {plain = true})
            io.replace("CMakeLists.txt", 'set(CMAKE_CXX_FLAGS "")', "",
                       {plain = true})
        end
