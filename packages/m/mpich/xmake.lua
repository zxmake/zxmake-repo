package("mpich", function()
    ...
    add_configs("device", {
        description = "Specify the communication device for MPICH.",
        default = "ofi",
        type = "string",
        values = {"ofi", "ucx"}
    })
    ...
    on_install("macosx", "linux", function(package)
        local configs = {
            "--disable-fortran", "--without-slurm", "--without-xpmem",
            "--without-hcoll", "--without-blcr", "--without-papi",
            "--without-pmix"
        }
        ...
        table.insert(configs, "--with-hwloc-prefix=" ..
                         package:dep("hwloc"):installdir())
        table.insert(configs,
                     "--with-x=" .. (package:config("x11") and "yes" or "no"))
