package("msys2", function()
    ...
    add_configs("msystem", {
        description = "Set msys2 system.",
        type = "string",
        values = {
            "MSYS", "MINGW32", "MINGW64", "UCRT64", "CLANG32", "CLANG64",
            "CLANGARM64"
        }
    })
    add_configs("pathtype", {
    ...
    add_configs("diffutils", {
        description = "Install diffutils.",
        default = false,
        type = "boolean"
    })
    add_configs("base_devel", {
        description = "Install base-devel.",
        default = false,
        type = "boolean"
    })
    add_configs("mingw64_gcc", {
        description = "Install mingw64 gcc.",
        default = false,
        type = "boolean"
    })
    add_configs("mingw64_toolchain", {
        description = "Install mingw64 toolchain.",
        default = false,
        type = "boolean"
    })
    add_configs("mingw32_gcc", {
        description = "Install mingw32 gcc.",
        default = false,
        type = "boolean"
    })
    add_configs("mingw32_toolchain", {
        description = "Install mingw32 toolchain.",
        default = false,
        type = "boolean"
    })
    ...
    on_test(function(package)
        ...
        if msystem then
            if msystem == "MINGW64" then
                if package:config("mingw64_gcc") or
                    package:config("mingw64_toolchain") then
                    os.vrun("x86_64-w64-mingw32-gcc --version")
                end
            elseif msystem == "MINGW32" then
                if package:config("mingw32_gcc") or
                    package:config("mingw32_toolchain") then
