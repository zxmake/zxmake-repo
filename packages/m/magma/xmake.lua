package("magma", function()
    ...
    add_urls(
        "http://icl.utk.edu/projectsfiles/magma/downloads/magma-$(version).tar.gz")
    add_versions("2.7.1",
                 "d9c8711c047a38cae16efde74bee2eb3333217fd2711e1e9b8606cbbb4ae1a50")

    add_patches("2.7.1", path.join(os.scriptdir(), "patches", "2.7.1",
                                   "disable_test.patch"),
                "a4cc8f7a19d5cce80bf358d69eedfd8642edc0b8a931c792be239b5298519835")
    ...
    add_configs("gpu_target", {
        description = "GPU architectures to compile for.",
        default = "sm_50 sm_70",
        type = "string"
    })
    add_configs("blas_vendor", {
        description = "BLAS vendor to use.",
        default = "OpenBLAS",
        type = "string",
        values = {
            "OpenBLAS", "Intel10_64lp", "Intel10_64lp_seq", "Intel10_64ilp",
            "Intel10_64ilp_seq", "Generic"
        }
    })
    ...
    on_install("windows", "linux", function(package)
        ...
        table.insert(configs, "-DUSE_FORTRAN=" ..
                         (package:config("fortran") and "ON" or "OFF"))
