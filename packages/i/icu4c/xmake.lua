package("icu4c", function()

    set_homepage("http://site.icu-project.org/")
    set_description("C/C++ libraries for Unicode and globalization.")

    add_urls(
        "https://github.com/unicode-org/icu/releases/download/release-$(version)-src.tgz",
        {
            version = function(version)
                return (version:gsub("%.", "-")) .. "/icu4c-" ..
                           (version:gsub("%.", "_"))
            end
        })
    add_versions("75.1",
    ...
    add_patches("69.1", path.join(os.scriptdir(), "patches", "69.1",
                                  "replace-py-3.patch"),
                "ae27a55b0e79a8420024d6d349a7bae850e1dd403a8e1131e711c405ddb099b9")
    add_patches("70.1", path.join(os.scriptdir(), "patches", "70.1",
                                  "replace-py-3.patch"),
                "6469739da001721122b62af513370ed62901caf43af127de3f27ea2128830e35")
    if is_plat("mingw") then
        add_patches(">=69.1", path.join(os.scriptdir(), "patches", "72.1",
                                        "mingw.patch"),
                    "9ddbe7f691224ccf69f8c0218f788f0a39ab8f1375cc9aad2cc92664ffcf46a5")
    end
    ...
    on_load(function(package)
        ...
        local libsuffix = package:is_debug() and
                              package:is_plat("mingw", "windows") and "d" or ""
        package:add("links", "icutu" .. libsuffix, "icuio" .. libsuffix)
        if package:is_plat("mingw", "windows") then
            package:add("links", "icuin" .. libsuffix, "icuuc" .. libsuffix,
                        "icudt" .. libsuffix)
        else
            package:add("links", "icui18n" .. libsuffix, "icuuc" .. libsuffix,
                        "icudata" .. libsuffix)
        end
    end)
    ...
    on_install("windows", function(package)
        ...
        if package:is_cross() then
            ...
            local configs = {
                sln, "/p:Configuration=Release", "/target:pkgdata,genrb"
            }
        ...
        local configs = {sln, "/p:SkipUWP=True", "/p:_IsNativeEnvironment=true"}
    ...
    on_install("macosx", "linux", "mingw@msys", function(package)
        ...
        if package:is_plat("linux") and package:config("pic") ~= false then
            envs = autoconf.buildenvs(package,
                                      {cxflags = "-fPIC", cxxflags = cxxflags})
        else
