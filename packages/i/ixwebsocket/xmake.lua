package("ixwebsocket", function()
    ...
    add_urls(
        "https://github.com/machinezone/IXWebSocket/archive/refs/tags/$(version).tar.gz",
        "https://github.com/machinezone/IXWebSocket.git")

    add_versions("v11.4.5",
                 "c5fc225edec32bf7d583e55347ef2c9c4940d005c13ef5e848354a85602f5fd6")
    add_versions("v11.4.4",
                 "9ef7fba86a91ce18693451466ddc54b1e0c4a7dc4466c3028d888d6d55dde539")
    ...
    add_configs("use_tls",
                {description = "Use TLS", default = false, type = "boolean"})
    ...
    on_install(function(package)
        ...
        table.insert(configs, "-DUSE_TLS=" ..
                         (package:config("use_tls") and "ON" or "OFF"))
        ...
        if is_plat("wasm") then
            io.replace("ixwebsocket/IXUserAgent.cpp",
                       [[ss << " " << PLATFORM_NAME]],
                       [[ss << " " << "unknown platform"]], {plain = true})
        end

        io.replace("ixwebsocket/IXSocketMbedTLS.cpp", [[/* errorMsg */]],
                   [[errorMsg]], {plain = true})
