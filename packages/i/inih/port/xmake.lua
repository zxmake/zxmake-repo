...
option("start_of_line_comment_prefix", {
...
if not has_config("utf_8_bom") then
    add_defines("INI_ALLOW_BOM=0")
end
if not has_config("inline_comments") then
    add_defines("INI_ALLOW_INLINE_COMMENTS=0")
end
if has_config("inline_comment_prefix") then
    add_defines([[INI_INLINE_COMMENT_PREFIXES="]] ..
                    get_config("inline_comment_prefix") .. [["]])
end
if has_config("start_of_line_comment_prefix") then
    add_defines([[INI_START_COMMENT_PREFIXES="]] ..
                    get_config("start_of_line_comment_prefix") .. [["]])
end
if has_config("allow_no_value") then
    add_defines("INI_ALLOW_NO_VALUE=1")
end
if has_config("stop_on_first_error") then
    add_defines("INI_STOP_ON_FIRST_ERROR=1")
end
if has_config("report_line_numbers") then
    add_defines("INI_HANDLER_LINENO=1")
end
if has_config("call_handler_on_new_section") then
    add_defines("INI_CALL_HANDLER_ON_NEW_SECTION=1")
end
...
if has_config("initial_malloc_size") then
    add_defines("INI_INITIAL_ALLOC=" .. get_config("initial_malloc_size"))
end
