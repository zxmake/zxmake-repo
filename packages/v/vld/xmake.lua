package("vld", function()
    ...
    add_versions("v2.7.0",
                 "1bb5695a424b234d29d16acdc6bdb4be79d58501674b6d3765a19f237c5ad0f2")
    ...
    on_install("windows|!arm64", function(package)
        ...
        os.cp(
            "setup/dbghelp/" .. (package:is_arch("x64") and "x64" or "x86") ..
                "/*", package:installdir("bin"))
        local outputdir = path.join("src", "bin", package:is_arch("x64") and
                                        "x64" or "Win32", (package:debug() and
                                        "Debug" or "Release-") .. "*")
    ...
    on_test(function(package)
        assert(package:has_cfuncs("VLDDisable", {
            includes = "vld.h",
            configs = {defines = "VLD_FORCE_ENABLE"}
        }))
    end)
end)
