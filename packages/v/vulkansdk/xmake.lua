package("vulkansdk", function()
    ...
    add_configs("utils", {
        description = "Enabled vulkan utilities.",
        default = {},
        type = "table"
    })
    ...
    on_fetch(function(package, opt)
        if opt.system then
            ...
            if vulkansdk then
                ...
                table.insert(utils, package:is_plat("windows", "mingw") and
                                 "vulkan-1" or "vulkan")

                for _, util in ipairs(utils) do
                    if not find_library(util, vulkansdk.linkdirs,
                                        {plat = package:plat()}) then
                        wprint(format("The library %s for %s is not found!",
                                      util, package:arch()))
                        return
                    end
