package("vulkanscenegraph", function()
    ...
    add_urls(
        "https://github.com/vsg-dev/VulkanSceneGraph/archive/refs/tags/VulkanSceneGraph-$(version).tar.gz",
        "https://github.com/vsg-dev/VulkanSceneGraph.git")
    add_versions("1.0.2",
                 "526acd58d6e3a3bd3c3169996e0616d5c4a01e793dc064e8d20217791743bab5")
    ...
    on_install("windows", "macosx", "linux", function(package)
        io.replace("CMakeLists.txt", "vsg_add_target_clobber()", "",
                   {plain = true})
        io.replace("CMakeLists.txt", "vsg_add_target_uninstall()", "",
                   {plain = true})
        local configs = {}
    ...
    on_test(function(package)
        assert(package:check_cxxsnippets({
            ...
            includes = "vsg/utils/CommandLine.h"
        }))
    end)
end)
