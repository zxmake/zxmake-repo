package("vc-ltl5", function()
    ...
    add_urls(
        "https://github.com/Chuyu-Team/VC-LTL5/releases/download/v$(version)/VC-LTL-$(version)-Binary.7z")
    add_versions("5.1.1",
                 "71eb81ad7d5270cb2a247d6b1c5c01b8efb8f2c869d2e5222be8aafab2fc07de")
    add_versions("5.0.6",
                 "e406f829f75d59c34ee1e34cb6e994eb7db0810123ae7196499f26df88bc0a6f")
    add_versions("5.0.7",
                 "08555aca30b2f77a484534be0799cfed05bfdeb1d1e461d91576264d7123e687")
    add_versions("5.0.9",
                 "71a3099978ff3dc83fe8e2ebddf104e2f916d13cd10fb01fe02317ebc7fb02d9")
    ...
    add_configs("min_version", {
        description = "Windows Target Platform Min Version",
        default = default_min_version,
        type = "string"
    })
    add_configs("subsystem", {
        description = "Windows xp subsystem",
        default = "windows",
        type = "string",
        values = {"console", "windows"}
    })
    add_configs("clean_import", {
        description = "Do not use ucrt apiset, such as api-ms-win-crt-time-l1-1-0.dll (for geeks)",
        default = false,
        type = "boolean"
    })
    add_configs("openmp", {
        description = "Use openmp library",
        default = false,
        type = "boolean",
        readonly = true
    })
    add_configs("shared", {
        description = "Use vs_runtime",
        default = true,
        type = "boolean",
        readonly = true
    })
    ...
    on_load("windows", function(package)
        ...
        if vs and tonumber(vs) < 2015 then
            cprint(
                "${color.warning}vc-ltl5 only supports vc14.0 or later versions")
        end
        -- is xp?
        if package:config("min_version"):startswith("5") then
            ...
            if arch then
                local flag = format("/subsystem:%s,%s",
                                    package:config("subsystem"), arch)
    ...
    on_install("windows", function(package)
        ...
        if semver_min_version then
            ...
            cprint(
                "${color.warning}Invalid min_version, use default min_version")
        ...
        os.cp("TargetPlatform/header", package:installdir("include"),
              {rootdir = "TargetPlatform"})
        os.cp(bindir .. "/header", package:installdir("include"),
              {rootdir = "TargetPlatform"})
        ...
        assert(os.isdir(libdir),
               "The architecture is not supported in this version")
        ...
        if package:config("clean_import") and os.isdir(clean_import_dir) then
            ...
            os.mv("CleanImport/vc-ltl5.lib",
                  "CleanImport/vc-ltl5-CleanImport.lib")
            os.cd(old)
        end
    end)
