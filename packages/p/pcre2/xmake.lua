package("pcre2", function()
    ...
    add_urls(
        "https://github.com/PhilipHazel/pcre2/releases/download/pcre2-$(version)/pcre2-$(version).tar.gz",
        ...
        on_check("android", function(package)
            if package:is_arch("armeabi-v7a") then
                ...
                assert(ndkver and tonumber(ndkver) > 22,
                       "package(pcre2/armeabi-v7a): need ndk version > 22")
            end
        end)
    end
    ...
    on_install(function(package)
        ...
        io.replace("CMakeLists.txt", "OUTPUT_NAME pcre2%-(%w-)%-static",
                   "OUTPUT_NAME pcre2-%1")
        ...
        table.insert(configs, "-DPCRE2_SUPPORT_JIT=" ..
                         (package:config("jit") and "ON" or "OFF"))
        table.insert(configs, "-DPCRE2_STATIC_PIC=" ..
        ...
        if package:is_plat("windows") then
            table.insert(configs, "-DPCRE2_STATIC_RUNTIME=" ..
