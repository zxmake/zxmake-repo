...
if has_config("utils") then
    ...
    local apps = {
        "mview", "curview", "subdiv", "smoothing", "fairing",
        "parameterization", "decimation", "remeshing", "mpview"
    }
    if not is_plat("windows") then
        table.insert(apps, "mconvert")
    end
    for _, app in ipairs(apps) do
        ...
        if app == "mpview" then
            add_files("src/apps/MeshProcessingViewer.cpp")
        end
        target_end()
    end
