package("pango", function()
    ...
    add_urls(
        "https://gitlab.gnome.org/GNOME/pango/-/archive/$(version)/pango-$(version).tar.gz")
    add_urls("https://gitlab.gnome.org/GNOME/pango.git")
    add_versions("1.51.1",
                 "ea92cd570cdba62ca52cc0a7c9ea3cd311b6da3f0328a5aa8a4a81b0a74944a5")
    add_versions("1.50.3",
                 "4a8b0cf33d5f9ecaa9cd99dd72703d5c4c53bc58df64dd9538493bb4356ab691")
    ...
    on_install("windows|x64", "windows|x86", "macosx", "linux",
        ...
        io.replace("meson.build", "[ 'CoreFoundation', 'ApplicationServices' ]",
                   "[ 'CoreFoundation', 'ApplicationServices', 'Foundation' ]",
                   {plain = true})
        io.replace("meson.build", "dependency('gi-docgen'", "dependency(''",
                   {plain = true})
        io.replace("meson.build", "fallback: ['gi-docgen', 'dummy_dep']",
                   "fallback: ['dummy_dep']", {plain = true})

        -- fix unexpected -Werror=unused-but-set-variable errors, see https://gitlab.gnome.org/GNOME/pango/-/issues/693
        io.replace("meson.build", "'-Werror=unused-but-set-variable',", "",
                   {plain = true})
        ...
        local envs = meson.buildenvs(package, {
            packagedeps = {
                ...
                "glib", "pcre2", "libintl", "libiconv", "libthai", "libdatrie"
            }
        })
    ...
    on_test(function(package)
        assert(package:has_cfuncs("pango_layout_set_text",
                                  {includes = "pango/pangocairo.h"}))
    end)
end)
