    ...
    on_install("@linux", "@macosx", "@windows", function(package)
        ...
        if package:is_plat("linux", "macosx") then
            if linuxos.name() == "fedora" then
                ...
                io.replace("Bootstrap.mak", "-luuid", table.concat(
                               table.join(cflags, ldflags), " ") .. " -luuid")
                io.replace("Bootstrap.mak",
                           "$(MAKE) -C build/bootstrap -j`getconf _NPROCESSORS_ONLN` config=$(CONFIG)",
                           "", {plain = true})
                import("package.tools.make").build(package, configs)
                import("package.tools.make").build(package, {
                    "-C", "build/bootstrap", "configs=release",
                    "CPPFLAGS=" .. table.concat(cflags, " "),
        ...
        os.mv("bin/release/premake5" ..
                  (package:is_plat("windows") and ".exe" or ""),
