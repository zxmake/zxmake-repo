package("poppler", function()
    ...
    if is_plat("linux") then
        ...
        add_deps("freetype",
                 {configs = {woff2 = false, png = false, bzip2 = false}})
    end
    ...
    on_install("windows", "macosx", "linux", function(package)
        ...
        io.replace("CMakeLists.txt", "set(ENABLE_LIBPNG ON)\n",
                   ...
                   {plain = true})
        local configs = {
            "-DENABLE_UNSTABLE_API_ABI_HEADERS=ON", "-DBUILD_GTK_TESTS=OFF",
            "-DBUILD_QT5_TESTS=OFF", "-DBUILD_QT6_TESTS=OFF",
            "-DBUILD_CPP_TESTS=OFF", "-DENABLE_CPP=OFF", "-DENABLE_UTILS=OFF"
        }
        ...
        for _, libname in ipairs(libnames) do
            table.insert(configs, "-DENABLE_" .. libname:upper() .. "=" ..
                             (package:config(libname) and "ON" or "OFF"))
        end
    ...
    on_test(function(package)
        assert(package:has_cxxtypes("PDFDocBuilder", {
            configs = {languages = "c++11"},
            includes = "PDFDocBuilder.h"
        }))
    end)
end)
