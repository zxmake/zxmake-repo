package("pagmo", function()
    ...
    add_urls(
        "https://github.com/esa/pagmo2/archive/refs/tags/$(version).tar.gz",
        "https://github.com/esa/pagmo2.git")

    add_versions("v2.19.1",
                 "ecc180e669fa6bbece959429ac7d92439e89e1fd1c523aa72b11b6c82e414a1d")
    add_versions("v2.19.0",
                 "701ada528de7d454201e92a5d88903dd1c22ea64f43861d9694195ddfef82a70")
    add_versions("v2.18.0",
                 "5ad40bf3aa91857a808d6b632d9e1020341a33f1a4115d7a2b78b78fd063ae31")

    local configdeps = {
        eigen = "EIGEN3",
        nlopt = "NLOPT" --[[ipopt = "IPOPT"]]
    }
    for config, dep in pairs(configdeps) do
        add_configs(config, {
            description = "Enable features against " .. config .. ".",
            default = true,
            type = "boolean"
        })
    end
    ...
    on_install("windows", "macosx", "linux", "mingw@windows", "mingw@msys",
               function(package)
        local configs = {
            "-DCMAKE_INSTALL_LIBDIR=lib", "-DPAGMO_BUILD_TESTS=OFF",
            "-DBoost_USE_STATIC_LIBS=ON"
        }
        table.insert(configs, "-DCMAKE_BUILD_TYPE=" ..
                         (package:is_debug() and "Debug" or "Release"))
        table.insert(configs, "-DPAGMO_BUILD_STATIC_LIBRARY=" ..
                         (package:config("shared") and "OFF" or "ON"))
        for config, dep in pairs(configdeps) do
            table.insert(configs, "-DPAGMO_WITH_" .. dep .. "=" ..
