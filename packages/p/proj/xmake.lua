package("proj", function()
    ...
    add_configs("apps", {
        description = "Build PROJ applications.",
        default = false,
        type = "boolean"
    })
    add_configs("tiff", {
    ...
    on_install("!wasm and (!android or android@!windows)", function(package)
        ...
        if package:is_plat("windows") and package:is_arch("arm64") then
            io.replace("CMakeLists.txt", "add_subdirectory(data)", "",
                       {plain = true})
        end
        if package:config("curl") and (package:version():le(9.4)) then
            io.replace("src/lib_proj.cmake", "${CURL_LIBRARIES}",
                       "CURL::libcurl", {plain = true})
        end
        local configs = {
            "-DNLOHMANN_JSON_ORIGIN=external", "-DBUILD_TESTING=OFF"
        }
        ...
        table.insert(configs, "-DENABLE_TIFF=" ..
                         (package:config("tiff") and "ON" or "OFF"))
        table.insert(configs, "-DENABLE_CURL=" ..
                         (package:config("curl") and "ON" or "OFF"))
        table.insert(configs, "-DBUILD_PROJSYNC=" ..
                         (package:config("curl") and "ON" or "OFF"))
