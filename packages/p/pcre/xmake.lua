package("pcre", function()
    set_homepage("https://www.pcre.org/")
    set_description("A Perl Compatible Regular Expressions Library")
    set_license("BSD-3-Clause")

    set_urls(
        "https://github.com/xmake-mirror/pcre/releases/download/$(version)/pcre-$(version).tar.bz2")
    add_versions("8.45",
    ...
    add_configs("jit",
                {description = "Enable jit.", default = true, type = "boolean"})
    ...
    on_install("windows", function(package)
        ...
        table.insert(configs, "-DPCRE_SUPPORT_JIT=" ..
    ...
    on_test(function(package)
        local bitwidth = package:config("bitwidth") or "8"
        local testfunc = string.format("pcre%s_compile",
                                       bitwidth ~= "8" and bitwidth or "")
