package("promise-cpp", function()
    set_homepage("https://github.com/xhawk18/promise-cpp")
    set_description("C++ promise/A+ library in Javascript style.")
    set_license("MIT")

    add_urls(
        "https://github.com/xhawk18/promise-cpp/archive/refs/tags/$(version).tar.gz",
        "https://github.com/xhawk18/promise-cpp.git")

    add_versions("2.1.5",
                 "9608686d0d136323396a84c2f6046060a966ed10bf4bb7895ef87340c118b66a")
    add_versions("2.1.3",
                 "831f5c7fb36a1f0adda408898038b428d4afe96e7028947be0f755c6851eec26")

    add_configs("boost_asio", {
        description = "Enable boost asio.",
        default = false,
        type = "boolean"
    })
    ...
    on_install(function(package)
        ...
        import("package.tools.xmake").install(package, {
            boost_asio = package:config("boost_asio")
        })
    end)
    ...
    on_test(function(package)
        if package:config("boost_asio") then
            ...
            assert(package:has_cxxtypes("promise::Promise", {
                configs = {languages = "c++14"},
                includes = "promise-cpp/promise.hpp"
            }))
        end
    end)
end)
