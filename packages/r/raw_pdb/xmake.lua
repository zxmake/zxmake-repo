package("raw_pdb", function()
    ...
    on_install("!macosx and !iphoneos", function(package)
        ...
        if package:version():ge("2024.08.27") then
            io.replace("src/CMakeLists.txt", "if (UNIX)", "if(1)",
                       {plain = true})
            ...
            io.replace("src/CMakeLists.txt", "add_subdirectory(Examples)", "",
                       {plain = true})
    ...
    on_test(function(package)
        if package:version():ge("2024.08.27") then
            assert(package:has_cxxfuncs("PDB::ValidateFile(0, 0)", {
                includes = {"cstddef", "raw_pdb/PDB.h"}
            }))
        else
            assert(package:has_cxxfuncs("PDB::ValidateFile(0)",
                                        {includes = "PDB.h"}))
        end
    end)
end)
