package("remotery", function()
    ...
    add_configs("cuda", {
        description = "Assuming CUDA headers/libs are setup, allow CUDA profiling",
        default = false,
        type = "boolean"
    })
    add_configs("dx11", {
        description = "Assuming Direct3D 11 headers/libs are setup, allow D3D11 GPU profiling",
        default = false,
        type = "boolean"
    })
    add_configs("dx12", {
        description = "Allow D3D12 GPU profiling",
        default = false,
        type = "boolean"
    })
    add_configs("opengl", {
        description = "Allow OpenGL GPU profiling (dynamically links OpenGL libraries on available platforms)",
        default = false,
        type = "boolean"
    })
    add_configs("metal", {
        description = "Allow Metal profiling of command buffers",
        default = false,
        type = "boolean"
    })
    ...
    on_install("windows|x64", "linux", "macosx", "bsd", "mingw|x86_64", "msys",
        ...
        io.replace("lib/Remotery.c", [[#pragma comment(lib, "ws2_32.lib")]], "",
                   {plain = true})
        io.replace("lib/Remotery.c", [[#pragma comment(lib, "winmm.lib")]], "",
                   {plain = true})
