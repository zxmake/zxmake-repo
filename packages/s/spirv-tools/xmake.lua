package("spirv-tools", function()
            ...
            if result and not result.includedirs then
                for _, linkdir in ipairs(result.linkdirs) do
                    if linkdir:startswith("/usr") then
                        local includedir =
                            path.join(path.directory(linkdir), "include",
                                      "spirv-tools")
    ...
    on_install(function(package)
        ...
        table.insert(configs, "-DSPIRV-Headers_SOURCE_DIR=" ..
                         spirv:installdir():gsub("\\", "/"))
        ...
        package:add("links", "SPIRV-Tools-link", "SPIRV-Tools-reduce",
                    "SPIRV-Tools-opt")
    ...
    on_test(function(package)
        ...
        assert(package:has_cxxfuncs("spvContextCreate", {
            configs = {languages = "c++17"},
            includes = "spirv-tools/libspirv.hpp"
        }))
    end)
end)
