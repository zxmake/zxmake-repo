package("sfml", function()
    ...
    if not is_plat("macosx") or not is_arch("arm.*") then
        add_versions("2.5.1",
                     "438c91a917cc8aa19e82c6f59f8714da353c488584a007d401efac8368e1c785")
    end

    add_versions("2.6.1",
                 "82535db9e57105d4f3a8aedabd138631defaedc593cab589c924b7d7a11ffb9d")
    add_versions("2.6.0",
                 "0c3f84898ea1db07dc46fa92e85038d8c449e3c8653fe09997383173de96bc06")

    add_configs("graphics", {
        description = "Use the graphics module",
        default = true,
        type = "boolean"
    })
    add_configs("window", {
        description = "Use the window module",
        default = true,
        type = "boolean"
    })
    add_configs("audio", {
        description = "Use the audio module",
        default = true,
        type = "boolean"
    })
    add_configs("network", {
        description = "Use the network module",
        default = true,
        type = "boolean"
    })
    if is_plat("windows", "mingw") then
        add_configs("main", {
            description = "Link to the sfml-main library",
            default = true,
            type = "boolean"
        })
    end
    ...
    on_component("window", function(package, component)
        ...
        if not package:config("shared") then
            ...
            elseif package:is_plat("macosx") then
                component:add("frameworks", "Foundation", "AppKit", "IOKit",
                              "Carbon")
            elseif package:is_plat("iphoneos") then
                component:add("frameworks", "Foundation", "UIKit",
                              "CoreGraphics", "QuartzCore", "CoreMotion")
            end
        end
    end)
    ...
    on_component("audio", function(package, component)
        ...
        if not package:config("shared") and package:is_plat("windows", "mingw") then
            component:add("links", "openal32", "flac", "vorbisenc",
    ...
    on_load("windows", "linux", "macosx", "mingw", function(package)
        ...
        if package:is_plat("linux") then
            if package:config("window") or package:config("graphics") then
                package:add("deps", "libx11", "libxcursor", "libxrandr",
                            "libxrender", "libxfixes", "libxext", "eudev")
    ...
    on_install("windows", "linux", "macosx", "mingw", function(package)
        ...
        if package:config("shared") then
            ...
            if package:config("window") and package:is_plat("linux") then
                ...
                for _, name in ipairs({
                    "libx11", "libxcursor", "libxrandr", "libxrender",
                    "libxfixes", "libxext"
                }) do
                ...
                if #libfiles > 0 then
                    ...
                    file:print("target_link_libraries(sfml-window PRIVATE " ..
                                   table.concat(libraries, " ") .. ")")
                    file:close()
                end
            end
        ...
        table.insert(configs, "-DSFML_BUILD_AUDIO=" ..
                         (package:config("audio") and "ON" or "OFF"))
        table.insert(configs, "-DSFML_BUILD_GRAPHICS=" ..
                         (package:config("graphics") and "ON" or "OFF"))
        table.insert(configs, "-DSFML_BUILD_WINDOW=" ..
                         (package:config("window") and "ON" or "OFF"))
        table.insert(configs, "-DSFML_BUILD_NETWORK=" ..
                         (package:config("network") and "ON" or "OFF"))
