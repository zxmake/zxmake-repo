...
option("complex", {
    default = false,
    showmenu = true,
    description = "Build with complex number support."
})
option("graphblas", {
    default = false,
    showmenu = true,
    description = "Build GraphBLAS module."
})
option("graphblas_static", {
    default = false,
    showmenu = true,
    description = "Build static GraphBLAS module."
})
...
on_buildcmd_file(function(target, batchcmds, sourcefile, opt)
    ...
    for _, res in ipairs(prod_defs) do
        ...
        local filename = table.concat(table.join({path.basename(sourcefile)},
                                                 res), "_") .. ".c"
        local sourcefile_cx = target:autogenfile(sourcefile, {
            rootdir = rootdir,
            filename = filename
        })
        ...
        if res ~= "none" then
            batchcmds:show_progress(opt.progress,
                                    "${color.build.object}compiling.%s %s with define %s",
                                    get_config("mode"), sourcefile, res)
            batchcmds:compile(sourcefile, objectfile,
                              {configs = {defines = res}})
        else
            batchcmds:show_progress(opt.progress,
                                    "${color.build.object}compiling.%s %s with no defines",
                                    get_config("mode"), sourcefile)
            batchcmds:compile(sourcefile, objectfile)
        end
    end
...
for _, module in ipairs({
    "Core", "Check", "Cholesky", "MatrixOps", "Partition", "Modify",
    "Supernodal"
}) do
    add_files("CHOLMOD/" .. module .. "/cholmod*.c",
              {prod_defs = {"none", "DLONG"}})
end
...
local doubledefs = {
    "umf_analyze.c", "umf_apply_order.c", "umf_cholmod.c", "umf_colamd.c",
    "umf_free.c", "umf_fsize.c", "umf_is_permutation.c", "umf_malloc.c",
    "umf_realloc.c", "umf_report_perm.c", "umf_singletons.c"
}
for _, file in ipairs(doubledefs) do
    add_files("UMFPACK/Source/" .. file, {prod_defs = {"DINT", "DLONG"}})
end
add_files("UMFPACK/Source/*.c|umfpack_timer.c|umfpack_tictoc.c|" ..
              table.concat(doubledefs, "|"),
          {prod_defs = {"DINT", "DLONG", "ZINT", "ZLONG"}})
...
if has_config("graphblas") then
    if has_config("graphblas_static") then
        ...
        add_includedirs("GraphBLAS/Include", "GraphBLAS/Source",
                        "GraphBLAS/Source/Template",
                        "GraphBLAS/Source/Generated")
...
if has_config("with_cuda") then
    ...
    add_headerfiles("GPUQREngine/Include/(**.hpp)",
                    "GPUQREngine/Include/(**.cu)")
    target_end()
end
