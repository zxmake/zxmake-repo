package("suitesparse", function()
    ...
    add_urls(
        "https://github.com/DrTimothyAldenDavis/SuiteSparse/archive/refs/tags/$(version).tar.gz",
        "https://github.com/DrTimothyAldenDavis/SuiteSparse.git")
    add_versions("v7.8.1",
                 "b645488ec0d9b02ebdbf27d9ae307f705de2b6133edb64617a72c7b4c6c3ff44")
    add_versions("v7.7.0",
    ...
    add_versions("v5.10.1",
                 "acb4d1045f48a237e70294b950153e48dce5b5f9ca8190e86c2b8c54ce00a7ee")
    add_versions("v5.12.0",
                 "5fb0064a3398111976f30c5908a8c0b40df44c6dd8f0cc4bfa7b9e45d8c647de")
    add_versions("v5.13.0",
                 "59c6ca2959623f0c69226cf9afb9a018d12a37fab3a8869db5f6d7f83b6b147d")
    add_versions("v7.5.1",
                 "dccfb5f75aa83fe2edb4eb2462fc984a086c82bad8433f63c31048d84b565d74")

    add_patches("5.x",
                path.join(os.scriptdir(), "patches", "5.10.1", "msvc.patch"),
                "8ac61e9acfaa864a2528a14d3a7e6e86f6e6877de2fe93fdc87876737af5bf31")
    ...
    add_configs("blas", {
        ...
        default = "openblas",
        type = "string",
        values = {"mkl", "openblas", "apple"}
    })
    add_configs("blas_static", {
        description = "Use static BLAS library.",
        default = true,
        type = "boolean"
    })
    add_configs("graphblas", {
        description = "Enable GraphBLAS module.",
        default = not is_arch("x86"),
        type = "boolean"
    })
    add_configs("graphblas_static", {
        description = "Enable static GraphBLAS module.",
        default = false,
        type = "boolean"
    })
        ...
        if package:version():ge("7.4.0") then
            ...
            if package:is_plat("windows") and not package:config("shared") then
                ...
                if package:config("graphblas") then
                    package:add("links", "graphblas" ..
                                    (package:config("graphblas_static") and
                                        "_static" or ""))
                end
            end
            for _, lib in ipairs({
                "lagraphx", "lagraph", "graphblas", "spex", "spqr", "rbio",
                "ParU", "umfpack", "ldl", "klu", "klu_cholmod", "cxsparse",
                "cholmod", "colamd", "ccolamd", "camd", "btf", "amd",
                "suitesparse_mongoose", "suitesparseconfig"
            }) do
            ...
            if package:config("cuda") then
                package:add("deps", "cuda",
                            {system = true, configs = {utils = {"cublas"}}})
                package:add("links", "GPUQREngine")
                package:add("links", "SuiteSparse_GPURuntime")
            end
            for _, lib in ipairs({
                "SPQR", "UMFPACK", "LDL", "KLU", "CXSparse", "CHOLMOD",
                "COLAMD", "CCOLAMD", "CAMD", "BTF", "AMD", "suitesparseconfig"
            }) do
    ...
    on_install("windows|x64", "windows|x86", "macosx", "linux",
        ...
        if package:version():ge("7.4.0") then
            ...
            table.insert(configs, "-DGRAPHBLAS_BUILD_STATIC_LIBS=" ..
                             (package:config("graphblas_static") and "ON" or
                                 "OFF"))
            table.insert(configs, "-DSUITESPARSE_USE_OPENMP=" ..
                             (package:config("openmp") and "ON" or "OFF"))
            table.insert(configs, "-DSUITESPARSE_USE_CUDA=" ..
            ...
            table.insert(configs,
                         "-DBLA_VENDOR=" .. bla_vendor[package:config("blas")])
            table.insert(configs, "-DBLA_STATIC=" ..
                             (package:config("blas_static") and "ON" or "OFF"))
            if package:is_plat("windows") then
                if package:config("graphblas") then
                    table.insert(configs,
                                 "-DSUITESPARSE_ENABLE_PROJECTS=suitesparse_config;mongoose;amd;btf;camd;ccolamd;colamd;cholmod;cxsparse;ldl;klu;umfpack;paru;rbio;spqr;graphblas;lagraph") -- remove spex since it does not support windows
                else
                    table.insert(configs,
                ...
                local vs_sdkver =
                    import("core.tool.toolchain").load("msvc"):config(
                        "vs_sdkver")
                if vs_sdkver then
                    local build_ver = string.match(vs_sdkver,
                                                   "%d+%.%d+%.(%d+)%.?%d*")
                    assert(tonumber(build_ver) >= 18362,
                           "cpuinfo requires Windows SDK to be at least 10.0.18362.0")
                    table.insert(configs,
    ...
    on_test(function(package)
        if package:version():ge("7.4.0") then
            assert(package:has_cfuncs("SuiteSparse_start", {
                includes = "suitesparse/SuiteSparse_config.h"
            }))
        else
            assert(package:has_cfuncs("SuiteSparse_start",
                                      {includes = "SuiteSparse_config.h"}))
        end
    end)
end)
