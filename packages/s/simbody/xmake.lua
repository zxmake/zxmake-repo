package("simbody", function()
    ...
    add_urls(
        "https://github.com/simbody/simbody/archive/refs/tags/Simbody-$(version).tar.gz")
    add_versions("3.7",
                 "d371a92d440991400cb8e8e2473277a75307abb916e5aabc14194bea841b804a")

    add_patches("3.7",
                path.join(os.scriptdir(), "patches", "3.7", "msvc.patch"),
                "ac80c1bf9266d7d5e256a632ea1e2a00260dcc4cb68d2a23272fb309af61d2b7")
    ...
    on_install("windows", "macosx", "linux", function(package)
        ...
        if package:config("blas") == "openblas" then
            io.replace("CMakeLists.txt", "REALTIME_LIB rt",
                       "REALTIME_LIB rt gfortran", {plain = true})
        end
        io.replace("Simbody/Visualizer/simbody-visualizer/CMakeLists.txt",
                   "TEST_STATIC_TARGET})",
        ...
        local configs = {
            "-DINSTALL_DOCS=OFF", "-DBUILD_TESTING=OFF", "-DBUILD_EXAMPLES=OFF"
        }
        table.insert(configs, "-DCMAKE_BUILD_TYPE=" ..
                         (package:debug() and "Debug" or "Release"))
        table.insert(configs, "-DBUILD_STATIC_LIBRARIES=" ..
                         (package:config("shared") and "OFF" or "ON"))
        table.insert(configs, "-DBUILD_DYNAMIC_LIBRARIES=" ..
