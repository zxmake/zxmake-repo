package("sqlpp11", function()
    ...
    add_urls(
        "https://github.com/rbock/sqlpp11/archive/refs/tags/$(version).tar.gz",
        "https://github.com/rbock/sqlpp11.git")
    add_versions("0.64",
                 "72e6d37c716cc45b38c3cf4541604f16224aaa3b511d1f1d0be0c49176c3be86")
    add_versions("0.61",
                 "d5a95e28ae93930f7701f517b1342ac14bcf33a9b1c5b5f0dff6aea5e315bb50")

    add_deps("cmake")

    add_configs("sqlite3_connector", {
        description = "Enable SQlite3 connector.",
        default = false,
        type = "boolean"
    })
    add_configs("sqlcipher_connector", {
        description = "Enable SQlite3 connector with SQLCipher.",
        default = false,
        type = "boolean"
    })
    add_configs("mariadb_connector", {
        description = "Enable MariaDB connector.",
        default = false,
        type = "boolean"
    })
    add_configs("postgresql_connector", {
        description = "Enable PostgreSQL connector.",
        default = false,
        type = "boolean"
    })
    add_configs("mysql_connector", {
        description = "Enable MySQL connector.",
        default = false,
        type = "boolean"
    })
    ...
    on_install("windows", "linux", "macosx", function(package)
        ...
        if package:config("mysql_connector") then
            ...
            if libmysql then
                table.insert(configs, "-DMySQL_INCLUDE_DIR=" ..
                                 table.concat(
                                     libmysql.includedirs or
                                         libmysql.sysincludedirs, ";"))
                table.insert(configs, "-DMySQL_LIBRARY=" ..
                                 table.concat(libmysql.libfiles or {}, ";"))
            end
        end
