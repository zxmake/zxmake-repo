package("scotch", function()
    ...
    add_urls(
        "https://gitlab.inria.fr/scotch/scotch/-/archive/$(version)/scotch-$(version).zip",
        "https://gitlab.inria.fr/scotch/scotch.git")

    add_versions("v6.1.1",
                 "21d001c390ec63ac60f987b9921f33cc1967b41cf07567e22cbf3253cda8962a")
    add_versions("v7.0.5",
                 "fd52e97844115dce069220bacbfb45fccdf83d425614b02b67b44cedf9d72640")
    ...
    add_configs("zlib", {
        description = "Use ZLIB compression format.",
        default = true,
        type = "boolean"
    })
    add_configs("lzma", {
        description = "Use LZMA compression format.",
        default = false,
        type = "boolean"
    })
    add_configs("bz2", {
        description = "Use BZ2 compression format.",
        default = false,
        type = "boolean"
    })
    ...
    add_links("ptesmumps", "esmumps", "scotch", "scotcherr", "scotcherrexit",
              "scotchmetis", "scotchmetisv5", "scotchmetisv3")
    ...
    on_install("windows|x64", "windows|arm64", "linux", "macosx", "bsd",
               function(package)
        if package:gitref() or package:version():ge("7.0.0") then
            ...
            table.insert(configs, "-DUSE_ZLIB=" ..
                             (package:config("zlib") and "ON" or "OFF"))
            table.insert(configs, "-DUSE_LZMA=" ..
                             (package:config("lzma") and "ON" or "OFF"))
            table.insert(configs, "-DUSE_BZ2=" ..
                             (package:config("bz2") and "ON" or "OFF"))
        ...
        elseif package:is_plat("macosx", "linux") then
            ...
            elseif package:is_plat("linux") then
                ...
                os.cp(basename .. (package:config("shared") and ".shlib" or ""),
                      "Makefile.inc")
            end
            ...
            if zlib then
                ...
                for _, includedir in ipairs(
                                         zlib.sysincludedirs or zlib.includedirs) do
            ...
            make.make(package, {"prefix=" .. package:installdir(), "install"},
                      {envs = envs})
    ...
    on_test(function(package)
        assert(package:has_cfuncs("SCOTCH_graphInit", {
            includes = {"stdio.h", "stdlib.h", "scotch.h"}
        }))
    end)
end)
