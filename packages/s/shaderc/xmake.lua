package("shaderc", function()
    ...
    add_urls(
        "https://github.com/google/shaderc/archive/refs/tags/$(version).tar.gz",
        "https://github.com/google/shaderc.git")

    add_versions("v2024.1",
                 "eb3b5f0c16313d34f208d90c2fa1e588a23283eed63b101edd5422be6165d528")
    add_versions("v2024.0",
                 "c761044e4e204be8e0b9a2d7494f08671ca35b92c4c791c7049594ca7514197f")
    add_versions("v2022.2",
                 "517d36937c406858164673db696dc1d9c7be7ef0960fbf2965bfef768f46b8c0")
    ...
    on_install(function(package)
        ...
        io.replace("CMakeLists.txt", "add_subdirectory(third_party)", "",
                   {plain = true})
        io.replace("libshaderc_util/src/compiler.cc", "SPIRV/GlslangToSpv.h",
                   "glslang/SPIRV/GlslangToSpv.h", {plain = true})
        ...
        if not package:has_tool("sh", "link") then
            ...
            if package:version():ge("2023.8") then
                io.replace("libshaderc_util/CMakeLists.txt", "glslang SPIRV",
                           table.concat(links, " "), {plain = true})
            else
                io.replace("glslc/CMakeLists.txt",
                           "glslang OSDependent OGLCompiler HLSL glslang SPIRV",
                           "", {plain = true})
                io.replace("libshaderc_util/CMakeLists.txt",
                           "glslang OSDependent OGLCompiler HLSL glslang SPIRV",
                           table.concat(links, " "), {plain = true})
            end
            links = table.join({"shaderc", "shaderc_util"}, links)
            io.replace("glslc/CMakeLists.txt", "shaderc_util shaderc",
                       table.concat(links, " "), {plain = true})
        end
        -- glslc --version
        ...
        local configs = {
            "-DSHADERC_SKIP_EXAMPLES=ON", "-DSHADERC_SKIP_TESTS=ON",
        ...
        if package:is_plat("windows") then
            ...
            io.replace("CMakeLists.txt",
                       ...
                       "", {plain = true})
            table.insert(configs, "-DSHADERC_ENABLE_SHARED_CRT=" ..
    ...
    on_test(function(package)
        ...
        if not package:is_binary() then
            assert(package:has_cfuncs("shaderc_compiler_initialize",
                                      {includes = "shaderc/shaderc.h"}))
        end
    end)
end)
