package("x264", function()
    ...
    add_configs("cli",
                {description = "enable cli", default = false, type = "boolean"})
    add_configs("bashcompletion", {
        description = "enable installation of bash-completion script",
        default = false,
        type = "boolean"
    })
    add_configs("opencl", {
    ...
    add_configs("interlaced", {
        description = "enable interlaced encoding support",
        default = true,
        type = "boolean"
    })
    add_configs("bit-depth", {
        description = "set output bit depth",
        default = "all",
        value = {8, 10, "all"}
    })
    add_configs("chroma-format", {
        description = "output chroma format",
        default = "all",
        value = {400, 420, 422, 444, "all"}
    })
    ...
    add_configs("avs", {
        description = "enable avisynth support",
        default = false,
        type = "boolean"
    })
    add_configs("swscale", {
        description = "enable swscale support",
        default = false,
        type = "boolean"
    })
    add_configs("lavf", {
        description = "enable libavformat support",
        default = false,
        type = "boolean"
    })
    add_configs("ffms", {
        description = "enable ffmpegsource support",
        default = false,
        type = "boolean"
    })
    add_configs("gpac", {
        description = "enable gpac support",
        default = false,
        type = "boolean"
    })
    add_configs("lsmash", {
        description = "enable lsmash support",
        default = false,
        type = "boolean"
    })

    add_configs("toolchains", {
        readonly = true,
        description = "Set package toolchains only for cross-compilation."
    })
    ...
    on_load(function(package)
        if is_subhost("windows") and os.arch() == "x64" then
            local msystem = "MINGW" ..
                                (package:is_targetarch("i386", "x86", "i686") and
                                    "32" or "64")
            package:add("deps", "msys2",
                        {configs = {msystem = msystem, base_devel = true}})
        end
    ...
    on_install("!iphoneos and !bsd", function(package)
        if is_host("windows") then
            ...
            io.replace("Makefile",
                       ...
                       "$(INSTALL) -m 755 libx264.$(SOSUFFIX) $(DESTDIR)$(libdir)",
                       {plain = true})
        end
        ...
        local configs = {}
        table.insert(configs, "--enable-" ..
                         (package:config("shared") and "shared" or "static"))
        table.insert(configs, "--" ..
                         (package:is_debug() and "enable" or "disable") ..
                         "-debug")
        table.insert(configs, "--" ..
                         (package:config("lto") and "enable" or "disable") ..
                         "-lto")
        table.insert(configs, "--" ..
                         (package:config("pic") and "enable" or "disable") ..
                         "-pic")
        ...
        elseif package:is_plat("mingw") then
            ...
            table.insert(configs,
                         "--host=" .. (triples[package:arch()] or triples.i386))
    ...
    on_test(function(package)
        assert(package:has_cfuncs("x264_encoder_open",
                                  {includes = {"stdint.h", "x264.h"}}))
    end)
end)
