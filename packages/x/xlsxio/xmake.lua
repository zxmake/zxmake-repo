package("xlsxio", function()
    set_homepage("https://github.com/brechtsanders/xlsxio")
    set_description("XLSX I/O - C library for reading and writing .xlsx files")
    set_license("MIT")

    add_urls(
        "https://github.com/brechtsanders/xlsxio/archive/refs/tags/$(version).tar.gz",
        "https://github.com/brechtsanders/xlsxio.git")

    add_versions("0.2.35",
                 "03a4d1b1613953d46c8fc2ea048cd32007fbddcd376ab6d4156f72da2815adfa")
    add_versions("0.2.34",
                 "726e3bc3cf571ac20e5c39b1f192f3793d24ebfdeaadcd210de74aa1ec100bb6")
    ...
    add_configs("minizip_ng", {
        description = "Use Minizip NG",
        default = false,
        type = "boolean"
    })
    add_configs("wide", {
        description = "Also build UTF-16 library (libxlsxio_readw)",
        default = false,
        type = "boolean"
    })
    ...
    on_install("windows", "linux", "macosx", "mingw", "cross", function(package)
        ...
        package:add("defines", "BUILD_XLSXIO_" ..
    ...
    on_test(function(package)
        assert(package:has_cfuncs("xlsxioread_open",
                                  {includes = "xlsxio_read.h"}))
    end)
end)
