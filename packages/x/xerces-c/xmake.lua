package("xerces-c", function()
    ...
    add_urls(
        "https://archive.apache.org/dist/xerces/c/3/sources/xerces-c-$(version).zip")
    add_versions("3.2.5",
                 "4aa0f7ed265a45d253f900fa145cc8cae10414d085695f1de03a2ec141a3358b")
    add_versions("3.2.4",
                 "563a668b331ca5d1fc08ed52e5f62a13508b47557f88a068ad1db6f68e1f2eb2")
    ...
    local is_system_transcoder_supported =
        is_plat("linux", "windows", "mingw", "macosx")
    add_configs("transcoder", {
        description = "Transcoder (used to convert between internal UTF-16 and other encodings)",
        default = is_system_transcoder_supported and "system_transcoder" or
            "iconv",
        type = "string",
    ...
    on_check("android|armeabi-v7a", function(package)
        ...
        assert(ndk_sdkver and tonumber(ndk_sdkver) >= 26,
               "package(xerces-c): need ndk api level >= 26 for android armeabi-v7a")
    end)
    ...
    on_check(function(package)
        ...
        elseif package:config("mutex_manager") == "posix" then
            assert(
                (not package:is_plat("windows")) or package:is_plat("cygwin"),
                "UNIX and Cygwin only")
        end
    end)
    ...
    on_load(function(package)
        if package:config("transcoder") == "system_transcoder" then
            ...
            elseif package:is_plat("macosx") then
                ...
                raise(
                    "`system_transcoder` only support GNU iconv library, macOS APIs and Windows APIs!")
            end
        ...
        elseif package:config("network_accessor") == "winsock" then
            assert(package:is_plat("windows", "cygwin", "mingw"),
                   "Windows, Cygwin, MingGW only")
            add_syslinks("advapi32")
        end
    end)
    ...
    on_install("windows", "macosx", "linux", "android", function(package)
        ...
        if package:config("network_accessor") == "off" then
            ...
            table.insert(configs, "-Dnetwork-accessor=" ..
                             package:config("network_accessor"))
        end

        local packagedeps = {}
        if package:config("transcoder") == "iconv" or
            package:config("message_loader") == "iconv" then
            table.insert(packagedeps, "libiconv")
        end
        if package:config("transcoder") == "icu" or
            package:config("message_loader") == "icu" then
            table.insert(packagedeps, "icu4c")
            io.replace("cmake/FindICU.cmake",
    ...
    on_test(function(package)
        assert(package:check_cxxsnippets({
            ...
            includes = "xercesc/util/PlatformUtils.hpp"
        }))
    end)
end)
