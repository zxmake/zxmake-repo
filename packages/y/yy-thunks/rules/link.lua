...
on_config(function(target)
    ...
    if thunks then
        ...
        table.insert(target:objectfiles(),
                     path.join(installdir, "lib", objectfile))
    end
end)
...
after_link(function(target, opt)
    ...
    depend.on_changed(function()
        ...
        local editbin = assert(find_tool("editbin", {envs = msvc:runenvs()}),
                               "editbin not found!")

        -- osversion -> Major/Minor OperatingSystemVersion
        -- subsystem -> Major/Minor SubsystemVersion
        os.iorunv(editbin.program, {
            "/osversion:5.0", "/subsystem:console,5.0", target:targetfile()
        })
        progress.show(opt.progress, "${color.build.target}editing.$(mode) %s",
                      target:filename())
