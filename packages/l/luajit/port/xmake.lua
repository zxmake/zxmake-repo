...
before_build_files(function(target, sourcebatch)
    ...
    os.vrunv(buildvm_bin,
             {"-m", "bcdef", "-o", "src/lj_bcdef.h", unpack(sourcefiles)})
    os.vrunv(buildvm_bin,
             {"-m", "ffdef", "-o", "src/lj_ffdef.h", unpack(sourcefiles)})
    os.vrunv(buildvm_bin,
             {"-m", "libdef", "-o", "src/lj_libdef.h", unpack(sourcefiles)})
    os.vrunv(buildvm_bin,
             {"-m", "recdef", "-o", "src/lj_recdef.h", unpack(sourcefiles)})
    os.vrunv(buildvm_bin,
             {"-m", "vmdef", "-o", "src/lj_vmdef.h", unpack(sourcefiles)})
    os.vrunv(buildvm_bin,
             {"-m", "folddef", "-o", "src/lj_folddef.h", "src/lj_opt_fold.c"})
    if is_plat("windows", "mingw") then
...
add_files("src/lib_base.c", "src/lib_math.c", "src/lib_bit.c",
          "src/lib_string.c", "src/lib_table.c", "src/lib_io.c", "src/lib_os.c",
          "src/lib_package.c", "src/lib_debug.c", "src/lib_jit.c",
          "src/lib_ffi.c", "src/lib_buffer.c",
          {rules = {"buildvm", override = true}})
