    ...
    if on_check then
        on_check("android", function(package)
            ...
            assert(ndk and tonumber(ndk) > 22,
                   "package(libyuv): need ndk version > 22")
        end)
    end
    ...
    on_install("!cross", function(package)
        if package:is_plat("iphoneos") then
            io.replace("CMakeLists.txt",
                       ...
                       [[set(arch_lowercase "]] .. package:arch() .. [[")]],
                       {plain = true})
        end
        ...
        table.insert(configs, "-DLIBYUV_WITH_JPEG=" ..
                         (package:config("jpeg") and "ON" or "OFF"))
