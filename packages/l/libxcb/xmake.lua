local components = {
    composite = {
        apt_package = "apt::libxcb-composite0-dev",
        default_value = true
    },
    ...
    screensaver = {
        apt_package = "apt::libxcb-screensaver-dev",
        default_value = true
    },
    ...
    ["xfree86-dri"] = {
        apt_package = "apt::libxcb-xfreedri0-dev",
        default_value = true
    },
...
package("libxcb", function()
    ...
    add_versions("1.13.1",
                 "f09a76971437780a602303170fd51b5f7474051722bc39d566a272d2c4bde1b5")
    add_versions("1.14",
                 "2c7fcddd1da34d9b238c9caeda20d3bd7486456fc50b3cc6567185dbd5b0ad02")
    add_versions("1.16",
                 "bc0f75f84b28e6496a19a1d094d7e47def861a50cb7cce5b23b62eecdc2a4479")

    for name, opt in pairs(components) do
        add_configs(name, {
            description = format("Enable %s submodule (default is %s).", name,
                                 opt.default_value),
    ...
    on_install("macosx", "linux", "bsd", function(package)
        local configs = {
            ...
            "--enable-devel-docs=no", "--with-doxygen=no", "PYTHON=python3"
        }
        ...
        for name, opt in pairs(components) do
            table.insert(configs, format("--enable-%s=%s", name,
                                         package:config(name) and "yes" or "no"))
        end
        if package:is_plat("macosx") and package:is_cross() then
            import("package.tools.autoconf").install(package, configs, {
                cflags = "-arch " .. package:arch(),
                shflags = "-arch " .. package:arch()
            })
        else
            import("package.tools.autoconf").install(package, configs)
        end
