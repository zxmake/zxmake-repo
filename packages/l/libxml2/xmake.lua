package("libxml2", function()
    ...
    add_urls(
        "https://gitlab.gnome.org/GNOME/libxml2/-/archive/$(version)/libxml2-$(version).tar.bz2",
        "https://gitlab.gnome.org/GNOME/libxml2.git",
        "https://github.com/GNOME/libxml2.git")

    add_versions("v2.13.4",
                 "ba783b43e8b3475cbd2b1ef40474da6a4465105ee9818d76cd3ac7863550afce")
    add_versions("v2.13.2",
                 "5091cf2767c3f7ba08bf59fbe85b01db36ca21abdcb37deea964fcb26a4391fb")
    add_versions("v2.12.9",
                 "3cd02671b20954865f5b3e90f192d8cc4d815b2362c2ff9a0b450b41648dac58")
    add_versions("v2.11.9",
                 "b9ed243467f82306da608a7a735ed55d90c7aaab0d3c6cf61284c43daa77cfee")
    ...
    add_configs("all", {
        description = "Enable all configs, exclude python",
        default = false,
        type = "boolean"
    })
    ...
    if on_check then
        on_check("iphoneos", function(package)
            if not package:gitref() then
                raise(
                    "All version unsupported now. see https://gitlab.gnome.org/GNOME/libxml2/-/issues/774\nYou can use `libxml2 master` branch to build or open a pull request to patch it.")
            end
        end)
    end
    ...
    on_load(function(package)
        ...
        if package:config("python") then
            assert(package:config("shared"),
                   "package(libxml2) python interface require shared lib")
            if package:is_cross() then
                raise(
                    "libxml2 python interface does not support cross-compilation")
            end
    ...
    on_install(function(package)
        ...
        for name, _ in pairs(import("configs").get_libxml2_configs()) do
            ...
            table.insert(configs,
                         format("-DLIBXML2_WITH_%s=%s", name:upper(), enabled))
        end
        ...
        if package:is_plat("windows") then
            ...
            io.replace(pc, "-lxml2", "-l" .. path.basename(libfiles),
                       {plain = true})
        end
    end)

    on_test(function(package)
        assert(package:has_cfuncs("xmlInitParser",
                                  {includes = "libxml/parser.h"}))
        if package:config("python") then
            if package:is_plat("windows") then
