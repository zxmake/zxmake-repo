package("libxcursor", function()

    set_homepage("https://www.x.org/")
    set_description("X.Org: X Window System Cursor management library")

    set_urls(
        "https://www.x.org/archive/individual/lib/libXcursor-$(version).tar.gz")
    add_versions("1.2.1",
    ...
    if is_plat("macosx", "linux") then
        add_deps("pkg-config", "util-macros", "libx11", "libxfixes",
                 "libxrender")
    end
    ...
    on_test(function(package)
        assert(package:has_ctypes("XcursorFileHeader",
                                  {includes = "X11/Xcursor/Xcursor.h"}))
    end)
end)
