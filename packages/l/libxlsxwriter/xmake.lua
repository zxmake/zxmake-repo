package("libxlsxwriter", function()
    ...
    add_versions("1.1.5",
                 "12843587d591cf679e6ec63ecc629245befec2951736804a837696cdb5d61946")

    add_configs("tmpfile", {
        description = "Use the C standard library's tmpfile()",
        default = false,
        type = "boolean"
    })
    add_configs("md5", {
        description = "Build libxlsxwriter without third party MD5 lib",
        default = false,
        type = "boolean"
    })
    add_configs("openssl_md5", {
        description = "Build libxlsxwriter with the OpenSSL MD5 lib",
        default = false,
        type = "boolean"
    })
    add_configs("mem_file", {
        description = "Use fmemopen()/open_memstream() in place of temporary files",
        default = false,
        type = "boolean"
    })
    add_configs("64", {
        description = "Enable 64-bit filesystem support",
        default = true,
        type = "boolean"
    })
    add_configs("dtoa", {
        description = "Use the locale independent third party Milo Yip DTOA library",
        default = false,
        type = "boolean"
    })
    ...
    on_load(function(package)
        if package:is_plat("android") then
            ...
            assert(ndk_sdkver and tonumber(ndk_sdkver) > 21,
                   "package(libxlsxwriter): need ndk api level > 21 for android")
        end
    ...
    on_install(function(package)
        ...
        if package:is_plat("windows") then
            table.insert(configs, "-DUSE_STATIC_MSVC_RUNTIME=" ..
        ...
        table.insert(configs, "-DUSE_STANDARD_TMPFILE=" ..
                         (package:config("tmpfile") and "ON" or "OFF"))
        table.insert(configs, "-DUSE_NO_MD5=" ..
                         (package:config("md5") and "OFF" or "ON"))
        table.insert(configs, "-DUSE_OPENSSL_MD5=" ..
                         (package:config("openssl_md5") and "ON" or "OFF"))
        table.insert(configs, "-DUSE_MEM_FILE=" ..
                         (package:config("mem_file") and "ON" or "OFF"))
        table.insert(configs, "-DIOAPI_NO_64=" ..
                         (package:config("64") and "OFF" or "ON"))
        table.insert(configs, "-DUSE_DTOA_LIBRARY=" ..
                         (package:config("dtoa") and "ON" or "OFF"))

        io.replace("include/xlsxwriter/packager.h", "minizip/", "",
                   {plain = true})
        ...
        elseif package:is_plat("linux", "bsd", "mingw") then
            ...
            if package:version():le("1.1.5") then
                io.replace("CMakeLists.txt",
                           [[find_package(ZLIB REQUIRED "1.0")]],
                           "find_package(ZLIB REQUIRED)", {plain = true})
                io.replace("CMakeLists.txt",
                           [[find_package(MINIZIP REQUIRED "1.0")]], "",
                           {plain = true})
            else
