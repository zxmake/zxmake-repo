...
#endif]])
option_end()
set_configvar("ICONV_CONST",
              (has_config("HAVE_ICONV") and not has_config("ICONV_NO_CONST")) and
                  "const" or "", {quote = false})
...
configvar_check_ctypes("USE_MBSTATE_T", "mbstate_t",
...
void test() {environ.foo = 1;}]], {
    includes = has_config("__HAVE_UNISTD_H") and "unistd.h" or "stdlib.h",
    default = 1
})
...
add_configfiles("libcharset/(include/localcharset.h.build.in)",
                {filename = "localcharset.h", pattern = "@(.-)@"})
add_configfiles("libcharset/(include/libcharset.h.in)",
                {filename = "libcharset.h", pattern = "@(.-)@"})
...
after_install(function(target)
    os.cp("libcharset/include/libcharset.h.in",
          path.join(target:installdir(), "include", "libcharset.h"))
    os.cp("libcharset/include/localcharset.h.in",
          path.join(target:installdir(), "include", "localcharset.h"))
end)
target_end()
...
set_configdir(".")
set_configvar("DLL_VARIABLE", (is_plat("windows") and is_kind("shared")) and
                  "__declspec(dllimport)" or "")
add_configfiles("(include/iconv.h.build.in)", {
    filename = "iconv.h",
    pattern = "@(.-)@",
    variables = {EILSEQ = ""}
})
add_configfiles("(include/iconv.h.in)", {
    filename = "iconv.h.inst",
    pattern = "@(.-)@",
    variables = {EILSEQ = ""}
})
...
after_install(function(target)
    os.cp("include/iconv.h.inst",
          path.join(target:installdir(), "include", "iconv.h"))
    for _, name in ipairs(os.files("po/*.gmo")) do
        local locale = path.basename(name)
        os.cp(name,
              path.join(target:installdir(), "share", "locale", locale,
                        "LC_MESSAGES", "libiconv.mo"))
    end
end)
target_end()
...
end) -- disable installation

target("iconv_no_i18n")
set_kind("binary")
add_deps("iconv", "icrt")
if has_config("installprefix") then
    add_defines("INSTALLDIR=\"" ..
                    path.join(get_config("installprefix"), "bin"):gsub("\\",
                                                                       "\\\\") ..
                    "\"")
    add_defines("LOCALEDIR=\"" ..
                    path.join(get_config("installprefix"), "share", "locale"):gsub(
                        "\\", "\\\\") .. "\"")
end
