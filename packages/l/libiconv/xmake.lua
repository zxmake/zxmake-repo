package("libiconv", function()
    ...
    if is_plat("macosx") then
        add_patches("1.15", path.join(os.scriptdir(), "patches", "1.15",
                                      "patch-utf8mac.diff"),
    ...
    on_fetch("macosx", "linux", function(package, opt)
        if opt.system then
            if package:is_plat("linux") and
                package:has_tool("cc", "gcc", "gxx", "clang", "clangxx") then
                return {} -- libiconv is already a part of glibc (GNU iconv)
            else
                return package:find_package("system::iconv",
                                            {includes = "iconv.h"})
            end
        end
    end)
    ...
    on_install("macosx", "linux", "bsd", "cross", "wasm", function(package)
        local configs = {
            "--disable-dependency-tracking", "--enable-extra-encodings",
            "--enable-relocatable"
        }
        ...
        os.vrunv("make", {
            "-f", "Makefile.devel",
            "CFLAGS=" .. (package:config("cflags") or "")
        })
        import("package.tools.autoconf").install(package, configs)
    end)

    on_test(function(package)
        if package:is_plat("linux", "bsd") or
            (package:is_plat("macosx") and not package:config("shared")) then
