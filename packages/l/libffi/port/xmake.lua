...
configvar_check_csnippets("HAVE_LONG_DOUBLE",
                          [[assert(sizeof(long double) > sizeof(double));]],
                          {includes = "assert.h", tryrun = true, default = 0})
configvar_check_csnippets("SIZEOF_DOUBLE", [[printf("%d", sizeof(double));]],
                          {tryrun = true, output = true, number = true})
configvar_check_csnippets("SIZEOF_LONG_DOUBLE",
                          [[printf("%d", sizeof(long double));]],
                          {tryrun = true, output = true, number = true})
configvar_check_csnippets("SIZEOF_SIZE_T", [[printf("%d", sizeof(size_t));]],
                          {tryrun = true, output = true, number = true})
...
on_buildcmd_file(function(target, batchcmds, sourcefile, opt)
    ...
    batchcmds:execv(cl.program, {
        "/nologo", "/EP", "/Iinclude", "/I" .. path.directory(sourcefile_or),
        sourcefile_or
    }, {envs = envs, stdout = sourcefile_cx})

    local objectfile = target:objectfile(sourcefile_cx)
    table.insert(target:objectfiles(), objectfile)

    batchcmds:show_progress(opt.progress,
                            "${color.build.object}compiling.$(mode) %s",
                            sourcefile_cx)
...
elseif is_arch("x86_64") then
    add_files("src/x86/ffi64.c", "src/x86/unix64.S", "src/x86/ffiw64.c",
              "src/x86/win64.S")
    add_includedirs("src/x86")
    add_headerfiles("src/x86/ffitarget.h")
elseif is_arch("x64") then
...
elseif is_arch("arm") then
    ...
    add_files(is_plat("windows") and "src/arm/sysv_msvc_arm32.S" or
                  "src/arm/sysv.S")
    add_includedirs("src/arm")
    add_headerfiles("src/arm/ffitarget.h")
elseif is_arch("arm64") then
    add_files("src/aarch64/ffi.c")
    add_files(is_plat("windows") and "src/aarch64/win64_armasm.S" or
                  "src/aarch64/sysv.S")
...
before_build(function(target)
    ...
    if semver.compare(target:version(), "v3.4.4") <= 0 then
        io.replace("include/ffi.h", "!defined FFI_BUILDING",
                   target:is_static() and "0" or "1", {plain = true})
    end
end)
