package("libremidi", function()
    ...
    add_urls(
        "https://github.com/jcelerier/libremidi/archive/refs/tags/$(version).tar.gz",
        "https://github.com/jcelerier/libremidi.git")

    add_versions("v4.5.0",
                 "2e884a4c826dd87157ee4fab8cd8c7b9dbbc1ddb804cb10ef0852094200724db")
    add_versions("v3.0",
                 "133b40396ca72e35d94cb0950199c9d123352951e4705971a9cd7606f905328a")
    ...
    add_configs("boost", {
        description = "Use boost::small_vector to pass midi bytes instead of std::vector to reduce allocations.",
        default = false,
        type = "boolean"
    })
    add_configs("jack", {
        description = "Enable JACK back-end",
        default = false,
        type = "boolean"
    })
    add_configs("slim_message", {
        description = "Use a fixed-size message format",
        default = "0",
        type = "string"
    })
    ...
    on_install(function(package)
        ...
        table.insert(configs, "-DLIBREMIDI_HEADER_ONLY=" ..
                         (package:config("header_only") and "ON" or "OFF"))
        table.insert(configs, "-DLIBREMIDI_NO_JACK=" ..
                         (package:config("jack") and "OFF" or "ON"))
        table.insert(configs, "-DLIBREMIDI_SLIM_MESSAGE=" ..
                         package:config("slim_message"))
        if package:config("boost") then
        ...
        io.replace("CMakeLists.txt", "          ARCHIVE DESTINATION lib/static",
                   "          ARCHIVE DESTINATION lib", {plain = true})
    ...
    on_test(function(package)
        ...
        assert(package:check_cxxsnippets({test = code}, {
            ...
            includes = {"libremidi/libremidi.hpp"}
        }))
    end)
end)
