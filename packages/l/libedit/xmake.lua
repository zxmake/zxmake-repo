package("libedit", function()
    ...
    add_configs("terminal_db", {
        description = "Select terminal library",
        default = "ncurses",
        type = "string",
        values = {"termcap", "ncurses", "tinfo"}
    })
    ...
    on_load(function(package)
        ...
        elseif terminal == "ncurses" then
            ...
            if package:is_plat("mingw") then
                raise(
                    "Unsupported ncurses on mingw, need package libsystre first")
            end
        else
    ...
    on_install("linux", "macosx", "bsd", "msys", function(package)
        ...
        if terminal_db == "ncurses" then
            ...
            if widec then
                -- hack
                io.replace("configure", "-lncurses", "-lncursesw",
                           {plain = true})
            end
        end
        import("package.tools.autoconf").install(package, configs,
                                                 {packagedeps = terminal_db})
    end)
