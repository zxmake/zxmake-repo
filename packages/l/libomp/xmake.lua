package("libomp", function()
    ...
    add_versions("11.1.0",
                 "d187483b75b39acb3ff8ea1b7d98524d95322e3cb148842957e9b0fbb866052e")
    add_versions("12.0.1",
                 "60fe79440eaa9ebf583a6ea7f81501310388c02754dbe7dc210776014d06b091")
    add_versions("19.1.0",
                 "c036fd95c8eeea8d7bce4196ee20cc5c5a99702d6ec7f0bb19828e315bc3a9ac")

    add_resources("19.1.0", "llvm_cmake",
    ...
    on_fetch("macosx", "linux", function(package, opt)
        if opt.system then
            return package:find_package("system::omp", {includes = "omp.h"}) or
                       package:find_package("system::omp5", {includes = "omp.h"})
        end
    end)
    ...
    on_install("macosx", "linux", "cross", function(package)
        ...
        if version:ge("19.0") then
            ...
            io.gsub("CMakeLists.txt", "set%(LLVM_COMMON_CMAKE_UTILS .-%)",
                    "set(LLVM_COMMON_CMAKE_UTILS \"" .. cmake .. "\")")
            io.replace("CMakeLists.txt", "include(OpenMPTesting)",
                       "function(add_openmp_testsuite target comment)\nreturn()\nendfunction()",
                       {plain = true})
            io.replace("CMakeLists.txt", "construct_check_openmp_target()", "",
                       {plain = true})
            io.replace("runtime/test/CMakeLists.txt",
                       "update_test_compiler_features",
                       "#update_test_compiler_features", {plain = true})
        end
        ...
        table.insert(configs,
                     "-DBUILD_SHARED_LIBS=" .. (shared and "ON" or "OFF"))
        table.insert(configs,
                     "-DLIBOMP_ENABLE_SHARED=" .. (shared and "ON" or "OFF"))
        table.insert(configs, "-DLIBOMP_OMPD_GDB_SUPPORT=" ..
