package("libxfixes", function()

    set_homepage("https://www.x.org/")
    set_description("X.Org: Header files for the XFIXES extension")

    set_urls(
        "https://www.x.org/archive/individual/lib/libXfixes-$(version).tar.gz")
    add_versions("6.0.0",
    ...
    on_test(function(package)
        assert(package:has_ctypes("XFixesSelectionNotifyEvent",
                                  {includes = "X11/extensions/Xfixes.h"}))
    end)
end)
