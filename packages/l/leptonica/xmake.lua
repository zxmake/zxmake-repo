package("leptonica", function()
    ...
    on_load("windows", function(package)
        ...
        if vs_sdkver then
            ...
            assert(tonumber(build_ver) ~= 17763,
                   "Unsupported Windows SDK 10.0.17763.0")
        end
    end)
    ...
    on_install("windows", "macosx", "linux", function(package)
        ...
        elseif package:is_plat("macosx") then
            io.replace("src/CMakeLists.txt", "${TIFF_LIBRARIES}", [[""]],
                       {plain = true})
        end

        io.replace("CMakeLists.txt", "NOT JP2K", "FALSE", {plain = true})
        local configs = {
            "-DSW_BUILD=OFF", "-DCMAKE_FIND_FRAMEWORK=LAST",
        ...
        local leptonica_cmake = package:installdir(
                                    "lib/cmake/leptonica/LeptonicaConfig.cmake")
        io.replace(leptonica_cmake, "if ()", "if (1)", {plain = true})
    end)

    on_test(function(package)
        assert(package:has_cfuncs("pixCleanBackgroundToWhite",
                                  {includes = "leptonica/allheaders.h"}))
    end)
end)
