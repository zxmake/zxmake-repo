package("libde265", function()
    ...
    add_versions("1.0.8",
                 "24c791dd334fa521762320ff54f0febfd3c09fc978880a8c5fbc40a88f21d905")
    ...
    on_install(function(package)
        ...
        table.insert(configs, "-DDISABLE_SSE=" ..
                         (package:is_arch("x86", "x64", "x86_64") and "OFF" or
                             "ON"))
        table.insert(configs, "-DENABLE_ENCODER=" ..
                         (package:config("tools") and "ON" or "OFF"))
        table.insert(configs, "-DENABLE_DECODER=" ..
                         (package:config("tools") and "ON" or "OFF"))
        ...
        if package:is_plat("windows") and package:is_debug() then
            ...
            if package:config("tools") then
                os.vcp(path.join(package:buildir(), "enc265/*.pdb"),
                       package:installdir("bin"))
                os.vcp(path.join(package:buildir(), "dec265/*.pdb"),
                       package:installdir("bin"))
            end
        end
    end)
    ...
    on_test(function(package)
        assert(package:has_cfuncs("de265_new_decoder",
                                  {includes = "libde265/de265.h"}))
    end)
end)
