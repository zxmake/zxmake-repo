package("libsoundio", function()
    ...
    add_versions("2.0.0",
                 "67a8fc1c9bef2b3704381bfb3fb3ce99e3952bc4fea2817729a7180fddf4a71e")

    add_includedirs("include", "include/soundio")

    add_configs("jack", {
        description = "Enable JACK backend.",
        default = false,
        type = "boolean"
    })
    ...
    add_configs("alsa", {
        description = "Enable Alsa backend.",
        default = false,
        type = "boolean"
    })
    add_configs("coreaudio", {
        description = "Enable CoreAudio backend.",
        default = false,
        type = "boolean"
    })
    add_configs("wasapi", {
        description = "Enable WASAPI backend.",
        default = false,
        type = "boolean"
    })
    ...
    on_load(function(package)
        ...
        elseif package:is_plat("macosx") and package:config("coreaudio") and
            not package:config("shared") then
            package:add("frameworks", "CoreAudio", "CoreFoundation",
                        "AudioToolbox")
        end
    end)

    on_install(function(package)
        io.gsub("src/config.h.in", "#cmakedefine SOUNDIO_HAVE_JACK",
                "${define SOUNDIO_HAVE_JACK}")
        io.gsub("src/config.h.in", "#cmakedefine SOUNDIO_HAVE_PULSEAUDIO",
                "${define SOUNDIO_HAVE_PULSEAUDIO}")
        io.gsub("src/config.h.in", "#cmakedefine SOUNDIO_HAVE_ALSA",
                "${define SOUNDIO_HAVE_ALSA}")
        io.gsub("src/config.h.in", "#cmakedefine SOUNDIO_HAVE_COREAUDIO",
                "${define SOUNDIO_HAVE_COREAUDIO}")
        io.gsub("src/config.h.in", "#cmakedefine SOUNDIO_HAVE_WASAPI",
                "${define SOUNDIO_HAVE_WASAPI}")
        io.gsub("src/config.h.in", "@LIBSOUNDIO_VERSION_MAJOR@",
                "${VERSION_MAJOR}")
        io.gsub("src/config.h.in", "@LIBSOUNDIO_VERSION_MINOR@",
                "${VERSION_MINOR}")
        io.gsub("src/config.h.in", "@LIBSOUNDIO_VERSION_PATCH@",
                "${VERSION_ALTER}")
        ...
        for _, name in ipairs({
            "jack", "pulseaudio", "alsa", "coreaudio", "wasapi"
        }) do
    ...
    on_test(function(package)
        assert(package:has_cfuncs("soundio_create",
                                  {includes = "soundio/soundio.h"}))
    end)
end)
