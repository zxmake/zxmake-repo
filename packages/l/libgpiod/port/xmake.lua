option("enable_bindings_cxx",
       {default = true, showmenu = true, description = "Enable C++ bindings"})
option("enable_tools",
       {default = true, showmenu = true, description = "Enable tools"})
...
if has_config("enable_bindings_cxx") then
    ...
    before_build(function(target)
        local configure = io.readfile("configure.ac")
        local version = configure:match(
                            "AC_INIT%(%[libgpiod%], %[?([0-9%.]+)%]?%)")
        target:add("defines", "GPIOD_VERSION_STR=\"" .. version .. "\"")
    end)
end
