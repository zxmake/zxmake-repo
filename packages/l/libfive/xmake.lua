package("libfive", function()
    ...
    on_install("windows", "macosx", "linux", function(package)
        if package:is_plat("windows") then
            io.replace("libfive/src/CMakeLists.txt", "EIGEN_INCLUDE_DIRS",
                       "EIGEN3_INCLUDE_DIRS", {plain = true})
            io.replace("CMakeLists.txt", "%/MD.", "")
        end
        io.replace("libfive/src/CMakeLists.txt",
                   "bash -c \"git diff --quiet --exit-code || echo +\"",
                   "git diff --quiet --exit-code", {plain = true})
        io.replace("libfive/src/CMakeLists.txt", "${GIT_REV}${GIT_DIFF}",
                   "${GIT_REV}+${GIT_DIFF}", {plain = true})
        io.replace("libfive/src/CMakeLists.txt", "if ?%(UNIX%)", "if (TRUE)")
        local configs = {
            "-DBUILD_GUILE_BINDINGS=OFF", "-DBUILD_PYTHON_BINDINGS=OFF",
            "-DBUILD_STUDIO_APP=OFF", "-DBUILD_TESTS=OFF"
        }
    ...
    on_test(function(package)
        assert(package:has_cfuncs("libfive_mesh_delete",
                                  {includes = "libfive.h"}))
    end)
end)
