package("libunwind", function()
    ...
    add_urls(
        "https://github.com/libunwind/libunwind/releases/download/$(version).tar.gz",
        {
    ...
    add_patches(">=1.8.0", path.join(os.scriptdir(), "patches", "1.8.0",
                                     "fix-arm64.patch"),
                "5f679af80859b6a50504ec830a4d328c3cf25bef9f2107baed866b438f3818d3")

    add_configs("minidebuginfo", {
        description = "Enables support for LZMA-compressed symbol tables",
        default = false,
        type = "boolean"
    })
    add_configs("zlibdebuginfo", {
        description = "Enables support for ZLIB-compressed symbol tables",
        default = false,
        type = "boolean"
    })
    ...
    on_install("android|arm64@linux,macosx", "linux", "bsd", "cross",
               function(package)
        if package:is_plat("bsd") then
            io.replace("src/setjmp/siglongjmp.c", "&wp[JB_MASK]",
                       "(unw_word_t)&wp[JB_MASK]", {plain = true})
        end
        ...
        table.insert(configs,
                     (package:config("minidebuginfo") and "--enable" or
                         "--disable") .. "-minidebuginfo")
        table.insert(configs,
                     (package:config("zlibdebuginfo") and "--enable" or
                         "--disable") .. "-zlibdebuginfo")
        import("package.tools.autoconf").install(package, configs)
    end)

    on_test(function(package)
        assert(package:has_cfuncs("_Unwind_Backtrace(0, 0)",
                                  {includes = "unwind.h"}))
    end)
end)
