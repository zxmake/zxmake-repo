package("libintl", function()
    ...
    add_versions("0.22.3",
                 "b838228b3f8823a6c1eddf07297197c4db13f7e1b173b9ef93f3f945a63080b6")
    ...
    on_fetch(function(package, opt)
        if opt.system then
            return
                package:find_package("system::intl", {includes = "libintl.h"})
        end
    end)
    ...
    on_install("windows", "macosx", "android", function(package)
        ...
        os.cp(path.join(os.scriptdir(), "port", package:version_str(),
                        "xmake.lua"), "xmake.lua")
        for _, conffile in ipairs({
            "gettext-runtime/config.h.in", "gettext-runtime/intl/config.h.in"
        }) do
