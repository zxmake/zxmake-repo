package("llfio", function()
    ...
    local hashes = {["2022.9.7"] = "ae7f9c5a92879285ad5100c89efc47ce1cb0031b"}
    add_urls(
        "https://github.com/ned14/llfio/archive/refs/tags/all_tests_passed_$(version).tar.gz",
        {
    ...
    add_configs("cpp20", {
        description = "Use C++20 version.",
        default = false,
        type = "boolean"
    })
    add_configs("enable_openssl", {
        description = "Enable openssl.",
        default = false,
        type = "boolean"
    })
    add_configs("experimental_status_code", {
        description = "Use experimental_status_code. (not supported atm)",
        default = false,
        type = "boolean",
        readonly
    })
    ...
    on_load("macosx", "iphoneos", "android", "linux", "windows",
            function(package)
        if package:config("headeronly") then
            package:add("defines", "LLFIO_HEADERS_ONLY=1")
            if package:is_plat("windows", "mingw") then
                package:add("syslinks", "advapi32", "user32", "wsock32",
    ...
    on_install("macosx", "iphoneos", "android", "linux", "windows",
               function(package)
        if package:is_plat("android") then
            ...
            assert(ndk_sdkver and tonumber(ndk_sdkver) >= 28,
                   "package(llfio): need ndk api level >= 28 for android")
        end
        local configs = {}
        configs.experimental_status_code = package:config(
                                               "experimental_status_code")
        ...
        io.replace("include/llfio/v2.0/detail/impl/posix/process_handle.ipp",
    ...
    on_test("macosx", "iphoneos", "android", "linux", "windows",
            function(package)
        local cxxflags = package:has_tool("cxx", "clang", "clangxx") and
                             {"-fsized-deallocation"} or {}
        local version = "17"
        if package:config("cpp20") then
            version = "20"
        end
