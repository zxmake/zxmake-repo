package("libxshmfence", function()

    set_homepage("https://www.x.org/")
    set_description("X.Org: Shared memory 'SyncFence' synchronization primitive")

    set_urls(
        "https://www.x.org/archive/individual/lib/libxshmfence-$(version).tar.bz2")
    add_versions("1.3",
    ...
    on_test(function(package)
        assert(package:has_ctypes("struct xshmfence",
                                  {includes = "X11/xshmfence.h"}))
    end)
end)
