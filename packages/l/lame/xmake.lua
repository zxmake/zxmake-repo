package("lame", function()
    ...
    on_install("windows", function(package)
        ...
        -- Honor vc runtime
        io.replace("Makefile.MSVC", "CC_OPTS = $(CC_OPTS) /MT", "",
                   {plain = true})
        -- Do not hardcode LTO
        io.replace("Makefile.MSVC", " /GL", "", {plain = true})
        io.replace("Makefile.MSVC", " /LTCG", "", {plain = true})
        io.replace("Makefile.MSVC", "ADDL_OBJ = bufferoverflowU.lib", "",
                   {plain = true})
        ...
        elseif package:is_arch("x64") then
            io.replace("Makefile.MSVC", "MACHINE = /machine:I386",
                       "MACHINE =/machine:X64", {plain = true})
            table.insert(configs, "MSVCVER=Win64")
            table.insert(configs, "asm=yes")
        elseif package:is_arch("arm64") then
            io.replace("Makefile.MSVC", "MACHINE = /machine:I386",
                       "MACHINE =/machine:ARM64", {plain = true})
            table.insert(configs, "MSVCVER=Win64")
        else
            table.insert(configs, "asm=yes")
        end
    ...
    on_test(function(package)
        assert(package:has_cfuncs("lame_encode_buffer",
                                  {includes = "lame/lame.h"}))
    end)
end)
