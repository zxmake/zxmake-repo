package("libatomic_ops", function()
    ...
    add_configs("gpl", {
        description = "Build atomic_ops_gpl library",
        default = false,
        type = "boolean"
    })
    ...
    on_install(function(package)
        ...
        table.insert(configs, "-Denable_gpl=" ..
                         (package:config("gpl") and "ON" or "OFF"))
        import("package.tools.cmake").install(package, configs)

        if package:config("shared") and
            package:is_plat("windows", "mingw", "cygwin") then
