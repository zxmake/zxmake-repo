package("libpfm", function()
    ...
    add_urls(
        "http://sourceforge.net/projects/perfmon2/files/libpfm4/libpfm-$(version).tar.gz")
    add_versions("4.11.0",
                 "5da5f8872bde14b3634c9688d980f68bda28b510268723cc12973eedbab9fecc")

    on_install("linux", function(package)
        if package:config("shared") then
            io.replace("lib/Makefile", "TARGETS=$(ALIBPFM)", "TARGETS=",
                       {plain = true})
            io.replace("lib/Makefile",
        ...
        table.insert(args,
                     "CONFIG_PFMLIB_DEBUG=" .. (package:debug() and "y" or "n"))
        table.insert(args, "CONFIG_PFMLIB_SHARED=" ..
                         (package:config("shared") and "y" or "n"))
    ...
    on_test(function(package)
        assert(package:has_cfuncs("pfm_initialize",
                                  {includes = "perfmon/pfmlib.h"}))
    end)
end)
