package("libverto", function()
    ...
    for _, cdep in ipairs(cdeps) do
        add_configs(cdep, {
            description = "Enable " .. cdep .. " support.",
            default = false,
            type = "boolean"
        })
    end
    ...
    on_install("macosx", "linux", function(package)
        ...
        for _, cdep in ipairs(cdeps) do
            table.insert(configs, "--with-" .. cdep .. "=" ..
                             (package:config(cdep) and "yes" or "no"))
        end
    ...
    on_test(function(package)
        assert(package:has_cfuncs("verto_get_supported_types",
                                  {includes = "verto-module.h"}))
    end)
end)
