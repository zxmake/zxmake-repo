package("libxinerama", function()

    set_homepage("https://www.x.org/")
    set_description("X.Org: API for Xinerama extension to X11 Protocol")

    set_urls(
        "https://www.x.org/archive/individual/lib/libXinerama-$(version).tar.gz")
    add_versions("1.1.5",
    ...
    on_test(function(package)
        assert(package:has_ctypes("XineramaScreenInfo",
                                  {includes = "X11/extensions/Xinerama.h"}))
    end)
end)
