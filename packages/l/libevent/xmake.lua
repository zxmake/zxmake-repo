package("libevent", function()
    ...
    add_configs("openssl", {
        description = "Build with OpenSSL library.",
        default = false,
        type = "boolean"
    })
    add_configs("mbedtls", {
        description = "Build with mbedtls library.",
        default = false,
        type = "boolean"
    })
    ...
    on_install("!android and (!mingw or mingw|!i386)", function(package)
        io.replace("CMakeLists.txt", "advapi32", "advapi32 crypt32",
                   {plain = true})
        if package:gitref() or package:version():eq("2.1.12") then
            io.replace("cmake/LibeventConfig.cmake.in",
                       ...
                       'get_filename_component(_INSTALL_PREFIX "${LIBEVENT_CMAKE_DIR}/../.." ABSOLUTE)',
                       {plain = true})
            io.replace("cmake/LibeventConfig.cmake.in", "NO_DEFAULT_PATH)", ")",
                       {plain = true})
        end

        local configs = {
            "-DEVENT__DISABLE_TESTS=ON", "-DEVENT__DISABLE_REGRESS=ON",
            "-DEVENT__DISABLE_SAMPLES=ON"
        }
        table.insert(configs, "-DCMAKE_BUILD_TYPE=" ..
                         (package:debug() and "Debug" or "Release"))
        table.insert(configs, "-DEVENT__LIBRARY_TYPE=" ..
                         (package:config("shared") and "SHARED" or "STATIC"))
        table.insert(configs, "-DEVENT__DISABLE_OPENSSL=" ..
                         (package:config("openssl") and "OFF" or "ON"))
        table.insert(configs, "-DEVENT__DISABLE_MBEDTLS=" ..
                         (package:config("mbedtls") and "OFF" or "ON"))
        if package:is_plat("windows") then
            table.insert(configs, "-DEVENT__MSVC_STATIC_RUNTIME=" ..
    ...
    on_test(function(package)
        assert(package:has_cfuncs("event_base_new",
                                  {includes = "event2/event.h"}))
    end)
end)
