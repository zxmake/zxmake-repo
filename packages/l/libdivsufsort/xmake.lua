package("libdivsufsort", function()
    ...
    add_urls(
        "https://android.googlesource.com/platform/external/libdivsufsort.git")
    ...
    add_deps("cmake")
    add_configs("use_64", {
        description = "Build 64bit suffxi array sorting APIs",
        default = false,
        type = "boolean"
    })
    ...
    on_test(function(package)
        if package:config("use_64") then
            assert(package:has_cfuncs("sa_search64",
                                      {includes = "divsufsort64.h"}))
        else
