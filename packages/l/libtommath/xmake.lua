package("libtommath", function()
    ...
    on_install(function(package)
        ...
        table.insert(configs, "-DCOMPILE_LTO =" ..
                         (package:config("lto") and "ON" or "OFF"))
        if package:is_plat("windows") and package:config("shared") then
            table.insert(configs, "-DCMAKE_WINDOWS_EXPORT_ALL_SYMBOLS=ON")
        end
        import("package.tools.cmake").install(package, configs)
        os.trymv(package:installdir("include/*.h"),
                 package:installdir("include/libtommath"))
    end)

    on_test(function(package)
        assert(package:has_cfuncs("mp_init", {includes = "tommath.h"}))
        assert(
            package:has_cfuncs("mp_init", {includes = "libtommath/tommath.h"}))
    end)
end)
