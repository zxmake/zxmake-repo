package("libfswatch", function()
    ...
    add_urls(
        "https://github.com/emcrisostomo/fswatch/archive/refs/tags/$(version).tar.gz",
        "https://github.com/emcrisostomo/fswatch.git")
    add_versions("1.17.1",
                 "bd492b6e203b10b30857778f4dd26f688426cd352937bd7779ee245139bafa2b")
    ...
    on_install("linux", "bsd", "macosx", function(package)
        ...
        io.replace("CMakeLists.txt", "add_subdirectory(test/src)", "",
                   {plain = true})
        io.replace("CMakeLists.txt", "add_subdirectory(fswatch/src)", "",
                   {plain = true})
        import("package.tools.cmake").install(package, configs)
    end)

    on_test(function(package)
        assert(package:has_cfuncs("fsw_add_path",
                                  {includes = "libfswatch/c/libfswatch.h"}))
    end)
end)
