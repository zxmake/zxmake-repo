package("libxmake", function()

    set_homepage("https://xmake.io")
    set_description("The c/c++ bindings of the xmake core engine")

    add_urls(
        "https://github.com/xmake-io/xmake/releases/download/$(version)/xmake-$(version).tar.gz")
    ...
    add_versions("v2.7.9",
                 "9b42d8634833f4885b05b89429dd60044dca99232f6096320b8d857fb33d2aef")
    ...
    on_install("linux", "macosx", "windows", function(package)
        ...
        os.cd("core")
        io.replace("xmake.lua", 'set_warnings("all", "error")', "",
                   {plain = true})
        io.replace("xmake.lua", [[option("pdcurses")
    set_default(true)
]], 'option("pdcurses")\nset_default(false)', {plain = true})
        io.replace("src/xmake/engine.c", 'sysarch = "arm64"',
                   'sysarch = "arm64";', {plain = true})
        io.replace("src/xmake/engine.c", 'sysarch = "arm"', 'sysarch = "arm";',
                   {plain = true})
...
typedef __int8 int8_t;]],
        ...
        if package:is_plat("linux", "macosx") and
            package:has_cfuncs("readline", {
                links = "readline",
    ...
    on_test(function(package)
        assert(
            package:has_cfuncs("xm_engine_init", {includes = "xmake/xmake.h"}))
    end)
end)
