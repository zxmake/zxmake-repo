package("libheif", function()
    ...
    add_versions("1.18.0",
                 "3f25f516d84401d7c22a24ef313ae478781b95f235c250b06152701c401055c3")
    add_versions("1.17.6",
    ...
    for _, conf in ipairs(configdeps) do
        add_configs(conf, {
            description = "Build " .. conf .. " encoder/decoder.",
            default = false,
            type = "boolean"
        })
    end
    ...
    on_install("windows", "macosx", "linux", function(package)
        local configs = {
            "-DWITH_EXAMPLES=OFF", -- TODO: package dep
            "-DWITH_AOM=OFF", "-DWITH_RAV1E=OFF", "-DWITH_LIBSHARPYUV=OFF"
        }
        ...
        for _, conf in ipairs(configdeps) do
            table.insert(configs, "-DWITH_" .. conf:upper() .. "=" ..
                             (package:config(conf) and "ON" or "OFF"))
        end
        import("package.tools.cmake").install(package, configs)
    end)

    on_test(function(package)
        assert(package:has_cfuncs("heif_get_version_number",
                                  {includes = "libheif/heif.h"}))
    end)
end)
