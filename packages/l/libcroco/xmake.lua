package("libcroco", function()
    ...
    add_urls("https://download.gnome.org/sources/libcroco/$(version).tar.xz", {
        version = function(version)
            return format("%d.%d/libcroco-%s", version:major(), version:minor(),
                          version)
        end
    })
    add_versions("0.6.13",
    ...
    on_load("windows", "macosx", "linux", function(package)
        ...
        package:add("includedirs",
                    format("include/libcroco-%d.%d", ver:major(), ver:minor()))
    end)
    ...
    on_test(function(package)
        assert(package:has_cfuncs("cr_utils_is_newline",
                                  {includes = "libcroco/libcroco.h"}))
    end)
end)
