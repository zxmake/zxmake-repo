package("libcoro", function()
    set_homepage("https://github.com/jbaldwin/libcoro")
    set_description("C++20 coroutine library")
    set_license("Apache-2.0")

    add_urls(
        "https://github.com/jbaldwin/libcoro/archive/refs/tags/$(version).tar.gz",
        "https://github.com/jbaldwin/libcoro.git", {submodules = false})

    add_versions("v0.15.0",
                 "9538281c742ca59c028052ad150d0e7ff688b8a724532dea95e74966b90d70c2")
    add_versions("v0.14.1",
                 "0a18058fe17826237a868e3d266960e839db8c7aeeb2beba9b596c84124afe0e")
    add_versions("v0.14.0",
                 "baf4b1535dee94bf47d3901b7e4842cedead5828ce7583e9a30ff8c5a8e0eb6e")
    add_versions("v0.13.0",
                 "aea5e4f4c04ef01269cc4e40ce9e693f71e324574ea0a933d908783ef385f9f5")
    ...
    if not is_plat("windows", "wasm") then
        ...
        add_configs("tls", {
            description = "Include TLS encryption features",
            default = false,
            type = "boolean"
        })
    end
    ...
    on_check(function(package)
        assert(package:check_cxxsnippets({
            ...
            #  endif
               ...
               "package(libcoro): Clang version too low, need at least 16.0.0")
        assert(package:check_cxxsnippets({
            test = [[
            ...
            #  endif
               ...
               "package(libcoro): GCC version too low, need at least 10.2.0")
        assert(package:check_cxxsnippets({
            test = [[
            ...
            #  endif
               ...
               "package(libcoro): Feature-test macro for jthread missing in <version>")
    end)
    ...
    on_install("!android", function(package)
        ...
        local configs = {
            "-DLIBCORO_EXTERNAL_DEPENDENCIES=ON", "-DLIBCORO_BUILD_TESTS=OFF",
            "-DLIBCORO_BUILD_EXAMPLES=OFF"
        }
        table.insert(configs, "-DCMAKE_BUILD_TYPE=" ..
                         (package:is_debug() and "Debug" or "Release"))
        table.insert(configs, "-DLIBCORO_BUILD_SHARED_LIBS=" ..
                         (package:config("shared") and "ON" or "OFF"))
        table.insert(configs, "-DLIBCORO_FEATURE_NETWORKING=" ..
                         (package:config("networking") and "ON" or "OFF"))
        table.insert(configs, "-DLIBCORO_FEATURE_TLS=" ..
                         (package:config("tls") and "ON" or "OFF"))
        local opt = {}
