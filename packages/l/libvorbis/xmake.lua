package("libvorbis", function()
    ...
    add_configs("vorbisenc", {
        description = "Includes vorbisenc",
        default = true,
        type = "boolean"
    })
    add_configs("vorbisfile", {
        description = "Includes vorbisfile",
        default = true,
        type = "boolean"
    })
        ...
        if opt.system then
            ...
            for _, name in ipairs(libs) do
                ...
                if not result then
                    ...
                    local includedirs = pkginfo.sysincludedirs or
                                            pkginfo.includedirs
    ...
    on_load(function(package)
        local ext = (package:is_plat("mingw") and package:config("shared")) and
                        ".dll" or ""
        if package:config("vorbisenc") then
    ...
    on_test(function(package)
        assert(package:has_cfuncs("vorbis_info_init",
                                  {includes = "vorbis/codec.h"}))
        if package:config("vorbisenc") then
            assert(package:has_cfuncs("vorbis_encode_init",
                                      {includes = "vorbis/vorbisenc.h"}))
        end
        if package:config("vorbisfile") then
            assert(package:has_cfuncs("ov_open_callbacks",
                                      {includes = "vorbis/vorbisfile.h"}))
        end
    end)
end)
