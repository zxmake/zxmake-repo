...
set_kind("$(kind)")
add_files("elf/*.cc")
add_headerfiles("elf/elf++.hh", "elf/data.hh", "elf/common.hh",
                {prefixdir = "libelfin/elf"})
...
on_load(function(target)
    ...
    local python = assert(import("lib.detect.find_tool")("python3"),
                          "python not found!")
    local result = try {
        function()
            return os.iorunv(python.program, {
                "elf/enum-print.py", "-u", "--hex", "--no-type", "--mask",
                "shf", "--mask", "pf", "-x", "loos", "-x", "hios", "-x",
                "loproc", "-x", "hiproc"
            }, {stdin = "elf/data.hh"})
        end
    }
...
set_kind("$(kind)")
add_files("dwarf/*.cc")
add_headerfiles("dwarf/dwarf++.hh", "dwarf/data.hh", "dwarf/small_vector.hh",
                {prefixdir = "libelfin/dwarf"})
...
on_load(function(target)
    ...
    local result = try {
        function()
            return os.iorunv(python.program, {"elf/enum-print.py"},
                             {stdin = "dwarf/dwarf++.hh"})
        end
    }
    if result then
        file:print(result)
    ...
    result = try {
        function()
            return os.iorunv(python.program, {
                "elf/enum-print.py", "-s", "_", "-u", "--hex", "-x", "hi_user",
                "-x", "lo_user"
            }, {stdin = "dwarf/data.hh"})
        end
    }
