package("libgit2", function()
    ...
    set_urls(
        "https://github.com/libgit2/libgit2/archive/refs/tags/$(version).tar.gz",
        "https://github.com/libgit2/libgit2.git")

    add_versions("v1.8.2",
                 "184699f0d9773f96eeeb5cb245ba2304400f5b74671f313240410f594c566a28")
    add_versions("v1.8.1",
    ...
    add_versions("v1.7.1",
                 "17d2b292f21be3892b704dddff29327b3564f96099a1c53b00edc23160c71327")
    add_versions("v1.3.0",
                 "192eeff84596ff09efb6b01835a066f2df7cd7985e0991c79595688e6b36444e")
    ...
    add_configs("https", {
        ...
        default = (is_plat("windows", "mingw") and "winhttp" or "openssl"),
        type = "string",
        values = {"winhttp", "openssl", "mbedtls"}
    })

    if is_plat("linux", "bsd") then
        add_syslinks("pthread", "dl")
    elseif is_plat("windows", "mingw") then
        add_syslinks("ole32", "rpcrt4", "winhttp", "ws2_32", "user32",
                     "crypt32", "advapi32")
    ...
    if on_check then
        on_check("windows", function(package)
            ...
            if package:is_arch("x86") and package:has_runtime("MD", "MDd") and
        ...
        on_check("android", function(package)
            if package:is_arch("armeabi-v7a") then
                ...
                assert(ndkver and tonumber(ndkver) > 22,
                       "package(libgit2) deps(pcre2) require ndk version > 22")
            end
        end)
    end
    ...
    on_install("!wasm", function(package)
        if package:is_plat("android") then
            for _, file in ipairs(os.files("src/**.txt")) do
                if path.basename(file) == "CMakeLists" then
                    io.replace(file, "C_STANDARD 90", "C_STANDARD 99",
                               {plain = true})
                end
            end
        elseif package:is_plat("windows") then
            -- MDd == _MT + _DLL + _DEBUG
            io.replace("cmake/DefaultCFlags.cmake", "/D_DEBUG", "",
                       {plain = true})
            ...
            if package:version():eq("1.7.1") then
                io.replace("cmake/DefaultCFlags.cmake", "/GL", "",
                           {plain = true})
            end
        end
        ...
        if https ~= "winhttp" then
            if package:is_plat("windows", "mingw", "msys") then
                ...
                io.replace("cmake/SelectHTTPSBackend.cmake", "elseif(WIN32)",
                           "elseif(0)", {plain = true})
            end

            if https == "mbedtls" then
                local links = {
                    "${MBEDTLS_LIBRARY}", "${MBEDX509_LIBRARY}",
                    "${MBEDCRYPTO_LIBRARY}"
                }
                if package:is_plat("windows", "mingw", "msys") then
        ...
        local configs = {
            "-DBUILD_TESTS=OFF", "-DBUILD_CLAR=OFF", "-DBUILD_EXAMPLES=OFF",
            "-DBUILD_FUZZERS=OFF", "-DREGEX_BACKEND=pcre2",
        ...
        table.insert(configs,
                     "-DUSE_SSH=" .. (package:config("ssh") and "ON" or "OFF"))
        table.insert(configs, "-DBUILD_CLI=" ..
        ...
        elseif package:is_plat("mingw") then
            ...
            local dlltool = assert(
                                os.files(path.join(mingw.bindir, "*dlltool*"))[1],
                                "dlltool not found!")
