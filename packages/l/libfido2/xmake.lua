package("libfido2", function()
    ...
    add_urls(
        "https://github.com/Yubico/libfido2/archive/refs/tags/$(version).tar.gz",
    ...
    add_configs("pcsc", {
        description = "Enable experimental PCSC support",
        default = false,
        type = "boolean"
    })
    add_configs("windows_hello", {
        description = "Abstract Windows Hello as a FIDO device",
        default = false,
        type = "boolean"
    })
    add_configs("nfc", {
        description = "Enable NFC support on Linux",
        default = false,
        type = "boolean"
    })
    add_configs("tools", {
    ...
    on_install("windows", "linux", "macosx", "bsd", "mingw", "msys",
        ...
        local configs = {
            "-DBUILD_TESTS=OFF", "-DBUILD_EXAMPLES=OFF", "-DBUILD_MANPAGES=OFF"
        }
        ...
        table.insert(configs, "-DUSE_HIDAPI=" ..
                         (package:config("hidapi") and "ON" or "OFF"))
        table.insert(configs, "-DUSE_PCSC=" ..
                         (package:config("pcsc") and "ON" or "OFF"))
        table.insert(configs, "-DUSE_WINHELLO=" ..
                         (package:config("windows_hello") and "ON" or "OFF"))
        table.insert(configs, "-DNFC_LINUX=" ..
                         (package:config("nfc") and "ON" or "OFF"))
        table.insert(configs, "-DBUILD_TOOLS=" ..
