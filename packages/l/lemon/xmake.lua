package("lemon", function()
    ...
    on_install("windows", "macosx", "linux", "mingw", "cross", function(package)
        local configs = {
            "-DLEMON_ENABLE_GLPK=OFF", "-DLEMON_ENABLE_ILOG=OFF",
            "-DLEMON_ENABLE_COIN=OFF", "-DLEMON_ENABLE_SOPLEX=OFF",
    ...
    on_test(function(package)
        assert(package:has_cxxtypes("lemon::SmartDigraph", {
            configs = {languages = "c++11"},
            includes = "lemon/smart_graph.h"
        }))
    end)
end)
