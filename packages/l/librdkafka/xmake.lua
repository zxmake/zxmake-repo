package("librdkafka", function()
    set_homepage("https://github.com/edenhill/librdkafka")
    set_description("The Apache Kafka C/C++ library")

    add_urls(
        "https://github.com/edenhill/librdkafka/archive/refs/tags/$(version).tar.gz",
        "https://github.com/edenhill/librdkafka.git")
    add_versions("v1.6.2",
                 "b9be26c632265a7db2fdd5ab439f2583d14be08ab44dc2e33138323af60c39db")
    add_versions("v1.8.2-POST2",
                 "d556d07cb88ea689e28c8e058ec3265ab333c9fc5e8f4ac0b7509bb5ae0e9f25")
    ...
    for config, dep in pairs(configdeps) do
        add_configs(config, {
            ...
            default = config_default[config] or false,
            type = "boolean"
        })
    end
    ...
    on_install(function(package)
        local configs = {
            "-DRDKAFKA_BUILD_EXAMPLES=OFF", "-DRDKAFKA_BUILD_TESTS=OFF",
            "-DWITH_BUNDLED_SSL=OFF"
        }
        table.insert(configs,
                     "-DWITH_SSL=" .. (package:config("ssl") and "ON" or "OFF"))
        table.insert(configs, "-DCMAKE_BUILD_TYPE=" ..
                         (package:debug() and "Debug" or "Release"))
        table.insert(configs, "-DRDKAFKA_BUILD_STATIC=" ..
                         (package:config("shared") and "OFF" or "ON"))
        for config, dep in pairs(configdeps) do
            table.insert(configs, "-DWITH_" .. config:upper() .. "=" ..
                             (package:config(config) and "ON" or "OFF"))
        end
        import("package.tools.cmake").install(package, configs)

        if package:version():startswith("v1.8.2") then
            io.replace(path.join(package:installdir("lib"), "cmake", "RdKafka",
                                 "RdKafkaConfig.cmake"), "find_dependency(LZ4)",
    ...
    on_test(function(package)
        assert(package:has_cfuncs("rd_kafka_produce",
                                  {includes = "librdkafka/rdkafka.h"}))
    end)
end)
