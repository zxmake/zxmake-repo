package("gettext", function()
    ...
    on_install("macosx", "linux", "android", function(package)
        local configs = {
            ...
            "--with-included-libunistring", "--with-lispdir=#{elisp}",
            "--disable-java", "--disable-csharp", "--without-emacs",
            "--without-git", "--without-bzip2", "--without-cvs", "--without-xz"
        }
        ...
        if package:is_plat("android") and package:version():le("0.20") then
            ...
            io.replace("./gettext-tools/gnulib-lib/spawn.in.h",
                       "@HAVE_SPAWN_H@", "0")
            io.replace("./gettext-tools/gnulib-lib/spawn.in.h",
            ...
            io.replace("./gettext-tools/src/Makefile.in",
                       ...
                       "bin_PROGRAMS =", {plain = true})
            io.replace("./gettext-tools/src/Makefile.in",
                       "cldr-plurals$(EXEEXT)", "", {plain = true})
            io.replace("./gettext-tools/src/Makefile.in", "install-exec-local:",
                       "install-exec-local: \n\ninstall-exec-local_: ",
                       {plain = true})
            io.replace("./gettext-tools/config.h.in", "#undef ICONV_CONST",
                       "#define ICONV_CONST const")
            io.replace("./gettext-runtime/config.h.in", "#undef ICONV_CONST",
                       "#define ICONV_CONST const")
            io.replace("./gettext-tools/libgrep/langinfo.in.h",
                       "@HAVE_LANGINFO_H@", "0")
            io.replace("./gettext-tools/gnulib-lib/langinfo.in.h",
                       "@HAVE_LANGINFO_H@", "0")
            io.replace("./gettext-runtime/gnulib-lib/langinfo.in.h",
                       "@HAVE_LANGINFO_H@", "0")
        end
        local cflags = {}
        local ldflags = {}
