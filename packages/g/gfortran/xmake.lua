    ...
    on_fetch(function(package, opt)
        ...
        if opt.system then
            ...
            if package:is_binary() then
                ...
                local installdir = path.directory(
                                       path.directory(fortran.program))
                local target
                ...
                if version then
                    ...
                    if target then
                        ...
                        table.insert(paths,
                                     path.join("/usr/lib/gcc", target, vmajor))
                        table.insert(paths, path.join(installdir, "lib", target,
                                                      vmajor))
                        if package:is_plat("macosx") then
                            table.insert(paths, path.join(
                                             "/opt/homebrew/Cellar/gcc",
                                             version, "/lib/gcc", vmajor))
                        end
                    end
