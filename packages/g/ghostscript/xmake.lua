package("ghostscript", function()
    ...
    add_urls(
        "https://github.com/ArtifexSoftware/ghostpdl-downloads/releases/download/$(version).tar.gz",
        {
            version = function(version)
                return format("gs%s/ghostscript-%s", version:gsub("%.", ""),
                              version)
            end
        })
    add_versions("9.55.0",
    ...
    on_install("windows|x64", "windows|x86", function(package)
        ...
        local vs_toolset = import("core.tool.toolchain").load("msvc"):config(
                               "vs_toolset")
        local vc_ver = "13"
        if vs == "2015" then
            vc_ver = "14"
        ...
        table.insert(configs,
                     "AROOTDIR=" .. package:installdir():gsub("\\", "/"))
