package("gdk-pixbuf", function()
    ...
    add_urls("https://download.gnome.org/sources/gdk-pixbuf/$(version).tar.xz",
             {
        alias = "home",
        version = function(version)
            return format("%d.%d/gdk-pixbuf-%s", version:major(),
    ...
    on_install("windows", "macosx", "linux", function(package)
        ...
        local configs = {
            "-Dman=false", "-Ddocs=false", "-Dgtk_doc=false", "-Dpng=enabled",
            "-Dtiff=enabled", "-Djpeg=enabled",
            "-Dnative_windows_loaders=false", "-Dbuiltin_loaders=all",
            "-Dgio_sniffing=false", "-Drelocatable=true",
            "-Dintrospection=disabled", "-Dtests=false",
        ...
        import("package.tools.meson").install(package, configs, {
            packagedeps = {
                "libjpeg-turbo", "libpng", "libtiff", "glib", "pcre2",
                "libintl", "libiconv"
            }
        })
    end)
    ...
    on_test(function(package)
        assert(package:has_cfuncs("gdk_pixbuf_get_type",
                                  {includes = "gdk-pixbuf/gdk-pixbuf.h"}))
    end)
end)
