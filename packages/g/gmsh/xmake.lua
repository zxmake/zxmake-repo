package("gmsh", function()
    ...
    on_install("windows", "macosx", "linux", function(package)
        io.replace("CMakeLists.txt", "add_definitions(-D_USE_MATH_DEFINES)",
                   "add_definitions(-DWIN32)\nadd_definitions(-D_USE_MATH_DEFINES)",
                   {plain = true})
        io.replace("CMakeLists.txt", "if(LINUX_JOYSTICK_H)",
                   "add_definitions(-DLINUX)\nif(LINUX_JOYSTICK_H)",
                   {plain = true})
        io.replace("contrib/metis/GKlib/GKlib.h", "#include <stddef.h>",
                   "#ifdef LINUX\n#include <strings.h>\n#endif\n#include <stddef.h>",
                   {plain = true})
        local configs = {"-DENABLE_TESTS=OFF", "-DENABLE_GMP=OFF"}
        table.insert(configs, "-DCMAKE_BUILD_TYPE=" ..
                         (package:debug() and "Debug" or "Release"))
        table.insert(configs, "-DENABLE_BUILD_LIB=" ..
                         (package:config("shared") and "OFF" or "ON"))
        table.insert(configs, "-DENABLE_BUILD_SHARED=" ..
                         (package:config("shared") and "ON" or "OFF"))
        table.insert(configs, "-DENABLE_BUILD_DYNAMIC=" ..
                         (package:config("shared") and "ON" or "OFF"))
        table.insert(configs, "-DENABLE_OPENMP=" ..
                         (package:config("openmp") and "ON" or "OFF"))
        if package:is_plat("windows") then
            table.insert(configs, "-DENABLE_MSVC_STATIC_RUNTIME=" ..
                             (package:has_runtime("MT", "MTd") and "ON" or "OFF"))
        end
        import("package.tools.cmake").install(package, configs)
        if package:is_plat("windows") and package:config("shared") then
            os.mv(path.join(package:installdir("lib"), "gmsh.dll"),
