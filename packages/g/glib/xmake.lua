package("glib", function()
    ...
    add_urls("https://download.gnome.org/sources/glib/$(version).tar.xz", {
        alias = "home",
        version = function(version)
            return format("%d.%d/glib-%s", version:major(), version:minor(),
                          version)
        end,
        excludes = {"*/COPYING"}
    })
    ...
    add_versions("home:2.71.0",
                 "926816526f6e4bba9af726970ff87be7dac0b70d5805050c6207b7bb17ea4fca")
    add_versions("home:2.78.1",
                 "915bc3d0f8507d650ead3832e2f8fb670fce59aac4d7754a7dab6f1e6fed78b2")
    add_patches("2.71.0",
                path.join(os.scriptdir(), "patches", "2.71.0", "macosx.patch"),
    ...
    on_fetch("macosx", "linux", function(package, opt)
        if opt.system and package.find_package then
            ...
            for _, name in ipairs({
                "gio-2.0", "gobject-2.0", "gthread-2.0", "gmodule-2.0",
                "glib-2.0"
            }) do
                local pkginfo = package.find_package and
                                    package:find_package("pkgconfig::" .. name,
                                                         opt)
                if pkginfo then
                    if not result then
                        ...
                        local includedirs =
                            pkginfo.sysincludedirs or pkginfo.includedirs
    ...
    on_install("windows", "macosx", "linux", "cross", function(package)
        local configs = {
            "-Dbsymbolic_functions=false", "-Ddtrace=false", "-Dman=false",
        ...
        table.insert(configs, "-Dglib_debug=" ..
                         (package:debug() and "enabled" or "disabled"))
        table.insert(configs, "-Ddefault_library=" ..
                         (package:config("shared") and "shared" or "static"))
        table.insert(configs, "-Dgio_module_dir=" ..
                         path.join(package:installdir(), "lib/gio/modules"))
        ...
        if package:is_plat("windows") then
            io.gsub("meson.build", "dependency%('libffi',",
                    "dependency('libffi', modules: ['libffi::ffi'],")
        end
        import("package.tools.meson").install(package, configs, {
            packagedeps = {"libintl", "libiconv", "libffi", "zlib"}
        })
