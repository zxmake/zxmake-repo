...
option("cross_platform_deterministic", {
    default = false,
    description = "Turns on behavior to attempt cross platform determinism. If this is set, JPH_USE_FMADD is ignored",
    defines = "JPH_CROSS_PLATFORM_DETERMINISTIC"
})
option("debug_renderer", {
    default = true,
...
option("profile", {
    default = false,
    description = "Turns on the internal profiler",
    defines = "JPH_PROFILE_ENABLED"
})
...
add_files("Jolt/**.cpp")
add_options("cross_platform_deterministic", "debug_renderer",
            "double_precision", "profile")
...
on_config(function(target)
    ...
    if is_arch("x86", "x64", "x86_64") then
        if target:has_tool("cxx", "cl") then
            ...
            if has_config("inst_fmadd") and
                not has_config("cross_platform_deterministic") then
                target:add("defines", "JPH_USE_FMADD")
            end
        elseif target:has_tool("cxx", "clang", "gcc") then
            if has_config("inst_avx512") then
                target:add("cxflags", "-mavx512f", "-mavx512vl", "-mavx512dq",
                           "-mavx2", "-mbmi", "-mpopcnt", "-mlzcnt", "-mf16c",
                           {force = true})
            elseif has_config("inst_avx2") then
                target:add("cxflags", "-mavx2", "-mbmi", "-mpopcnt", "-mlzcnt",
                           "-mf16c", {force = true})
            elseif has_config("inst_avx") then
    ...
    if is_plat("linux", "macosx", "mingw", "iphoneos", "wasm") then
        if target:has_tool("cxx", "gcc") then
            target:add("cxflags", "-Wno-comment", "-Wno-stringop-overflow",
                       "-ffp-contract=off", {force = true})
        else
