package("jansson", function()
    ...
    on_install("windows", "macosx", "linux", "mingw", function(package)
        local configs = {
            "-DJANSSON_EXAMPLES=OFF", "-DJANSSON_BUILD_DOCS=OFF",
            "-DJANSSON_WITHOUT_TESTS=ON"
        }
        table.insert(configs, "-DCMAKE_BUILD_TYPE=" ..
                         (package:debug() and "Debug" or "Release"))
        table.insert(configs, "-DJANSSON_BUILD_SHARED_LIBS=" ..
                         (package:config("shared") and "ON" or "OFF"))
        if package:is_plat("windows") then
            table.insert(configs, "-DJANSSON_STATIC_CRT=" ..
