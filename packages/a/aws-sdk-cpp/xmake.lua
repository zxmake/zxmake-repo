package("aws-sdk-cpp", function()
    ...
    add_configs("build_only", {
        description = 'By default, all SDKS are built, if only AWS S3 is required, then set build_only="s3", with multiple SDKS separated by commas.'
    })
    add_configs("http_client", {
        description = 'If disabled, no platform-default http client will be included in the library.',
        default = true,
        type = "boolean"
    })
    add_configs("encryption", {
        description = 'If disabled, no platform-default encryption will be included in the library.',
        default = false,
        type = "boolean"
    })
    ...
    on_install("linux", "macosx", function(package)
        ...
        table.insert(configs, "-DNO_HTTP_CLIENT=" ..
                         (package:config("http_client") and "OFF" or "ON"))
        table.insert(configs, "-DNO_ENCRYPTION=" ..
                         (package:config("encryption") and "OFF" or "ON"))
        if package:config("build_only") then
            table.insert(configs,
                         "-DBUILD_ONLY=" .. package:config("build_only"))
        end
