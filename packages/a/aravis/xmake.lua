package("aravis", function()
    ...
    add_urls(
        "https://github.com/AravisProject/aravis/releases/download/$(version)/aravis-$(version).tar.xz",
        {alias = "release"})
    add_urls(
        "https://github.com/AravisProject/aravis/archive/refs/tags/$(version).tar.gz",
        {alias = "github"})

    add_versions("release:0.8.33",
                 "3c4409a12ea70bba4de25e5b08c777112de854bc801896594f2cb6f8c2bd6fbc")

    add_versions("github:0.8.33",
                 "d70b125666b23ca4c0f8986fa0786a3d2b9efb7a56b558b703083cdfaa793f4e")

    add_configs("gst_plugin", {
    ...
    on_load(function(package)
        ...
        package:add("includedirs", "include", format("include/aravis-%d.%d",
                                                     version:major(),
                                                     version:minor()))
    end)

    on_install("windows|native", "macosx|native", "linux|native",
               function(package)
        ...
        table.insert(configs, "-Dgst-plugin=" ..
                         (package:config("gst_plugin") and "enabled" or
                             "disabled"))
        table.insert(configs, "-Dusb=" ..
                         (package:config("usb") and "enabled" or "disabled"))
        table.insert(configs, "-Dpacket-socket=" ..
                         (package:config("packet_socket") and "enabled" or
                             "disabled"))
        table.insert(configs, "-Dintrospection=" ..
                         (package:config("introspection") and "enabled" or
                             "disabled"))
