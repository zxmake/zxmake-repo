package("argtable3", function()
    ...
    on_install(function(package)
        ...
        if package:is_plat("mingw") then
            io.replace("src/version.rc.in", "#include <verrsrc.h>", "",
                       {plain = true})
        end
        ...
        local configs = {
            "-DARGTABLE3_ENABLE_TESTS=OFF", "-DARGTABLE3_ENABLE_EXAMPLES=OFF"
        }
        table.insert(configs, "-DCMAKE_BUILD_TYPE=" ..
                         (package:is_debug() and "Debug" or "Release"))
        table.insert(configs, "-DARGTABLE3_ENABLE_ARG_REX_DEBUG=" ..
