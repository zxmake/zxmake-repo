    ...
    if not packages then
        ...
        for _, packagedir in ipairs(os.dirs(path.join("packages", "*", "*"))) do
            ...
            if not pattern or packagename:match(pattern) then
                ...
                local instance = _load_package(packagename, packagedir,
                                               packagefile)
...
function _update_version(instance, version, shasum)
    ...
    if os.isfile(scriptfile) then
        io.gsub(scriptfile, "add_versions%(\"(.-)\",%s+\"(.-)\"%)",
                function(v, h)
        ...
        if versionfiles then
            for _, versionfile in ipairs(table.wrap(versionfiles)) do
                ...
                if os.isfile(versionfile) then
                    io.insert(versionfile, 1,
                              string.format("%s %s", version, shasum))
                    inserted = true
                end
            end
        end
    ...
    if inserted then
        local body = string.format(
                         "New version of %s detected (package version: %s, last github version: %s)",
                         instance:name(), version_current, version)
        os.vexec("git add .")
        os.vexec("git commit -a -m \"Update %s to %s\"", instance:name(),
                 version)
...
function main(pattern)
    ...
    while count < maxcount and #instances > 0 do
        ...
        local checkupdate_filepath = path.join(instance:scriptdir(),
                                               "checkupdate.lua")

        -- 优先使用 package 自定义的 checkupdate.lua, 如果不存在的话降级成全局的 scripts/checkupdate.lua
        ...
        if os.isfile(checkupdate_filepath) then
            local checkupdate = import("checkupdate", {
                rootdir = path.directory(checkupdate_filepath),
                anonymous = true
            })
            local version, shasum = checkupdate(instance)
            if version and shasum and not _is_pending(instance, version) then
                cprint(
                    "package(%s): new version ${bright}%s${clear} found, shasum: ${bright}%s",
                    instance:name(), version, shasum)
