...
function is_supported(instance, plat, arch, opt)
    ...
    local script =
        instance:get(instance:is_fetchonly() and "fetch" or "install")
...
function main(opt)
    ...
    for _, packagedir in ipairs(os.dirs(path.join("packages", "*", "*"))) do
        ...
        if instance and basename then
            local basedir = path.join("packages", basename:sub(1, 1):lower(),
                                      basename:lower())
        ...
        if instance then
            for _, plat in ipairs({
                "windows", "linux", "macosx", "iphoneos", "android", "mingw",
                "msys", "bsd", "wasm", "cross"
            }) do
    ...
    for _, packages_plat in pairs(packages) do
        table.sort(packages_plat, function(a, b)
